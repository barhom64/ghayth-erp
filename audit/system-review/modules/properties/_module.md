# وحدة `properties` — نظرة عامة

> الموجة: **2** | عدد الصفحات: **30** | أوراق منشأة: **30**

## KPIs
- مشاكل إجمالية: **26** (🔴 1 / ⚠ 3 / ℹ 22)

## الصفحات

| الصفحة | المسار | الكيان | سطور | API calls | أزرار | مشاكل |
|--------|--------|--------|------|-----------|--------|--------|
| [/properties/dashboard](./properties-dashboard.md) | `/properties/dashboard` | `dashboard` | 382 | 0 | 7 | 6 |
| [/properties/buildings/create](./properties-buildings-create.md) | `/properties/buildings/create` | `create` | 156 | 1 | 3 | 0 |
| [/properties/buildings/:id](./properties-buildings-byid.md) | `/properties/buildings/:id` | `:id` | 205 | 0 | 2 | 2 |
| [/properties/buildings](./properties-buildings.md) | `/properties/buildings` | `buildings` | 232 | 0 | 3 | 2 |
| [/properties/tenants/create](./properties-tenants-create.md) | `/properties/tenants/create` | `create` | 220 | 1 | 2 | 1 |
| [/properties/tenants/:id](./properties-tenants-byid.md) | `/properties/tenants/:id` | `:id` | 328 | 0 | 1 | 1 |
| [/properties/tenants](./properties-tenants.md) | `/properties/tenants` | `tenants` | 213 | 0 | 2 | 2 |
| [/properties/owners/create](./properties-owners-create.md) | `/properties/owners/create` | `create` | 125 | 0 | 3 | 1 |
| [/properties/owners/:id/edit](./properties-owners-byid-edit.md) | `/properties/owners/:id/edit` | `edit` | 158 | 0 | 2 | 1 |
| [/properties/owners/:id](./properties-owners-byid.md) | `/properties/owners/:id` | `:id` | 326 | 0 | 0 | 0 |
| [/properties/owners](./properties-owners.md) | `/properties/owners` | `owners` | 181 | 0 | 3 | 2 |
| [/properties/contracts/create](./properties-contracts-create.md) | `/properties/contracts/create` | `create` | 542 | 3 | 3 | 0 |
| [/properties/contracts/:contractId/pay/:installmentId](./properties-contracts-byid-pay-byid.md) | `/properties/contracts/:contractId/pay/:installmentId` | `:installmentId` | 120 | 0 | 3 | 0 |
| [/properties/contracts/:id](./properties-contracts-byid.md) | `/properties/contracts/:id` | `:id` | 326 | 0 | 2 | 0 |
| [/properties/contracts](./properties-contracts.md) | `/properties/contracts` | `contracts` | 372 | 0 | 2 | 2 |
| [/properties/payments/:paymentId/pay](./properties-payments-byid-pay.md) | `/properties/payments/:paymentId/pay` | `pay` | 131 | 1 | 3 | 0 |
| [/properties/payments/:id](./properties-payments-byid.md) | `/properties/payments/:id` | `:id` | 396 | 0 | 0 | 0 |
| [/properties/payments](./properties-payments.md) | `/properties/payments` | `payments` | 117 | 0 | 2 | 1 |
| [/properties/maintenance/create](./properties-maintenance-create.md) | `/properties/maintenance/create` | `create` | 118 | 1 | 3 | 0 |
| [/properties/maintenance/:id](./properties-maintenance-byid.md) | `/properties/maintenance/:id` | `:id` | 255 | 0 | 0 | 0 |
| [/properties/maintenance](./properties-maintenance.md) | `/properties/maintenance` | `maintenance` | 111 | 1 | 1 | 0 |
| [/properties/create](./properties-create.md) | `/properties/create` | `create` | 298 | 3 | 3 | 0 |
| [/properties/inspections](./properties-inspections.md) | `/properties/inspections` | `inspections` | 311 | 1 | 5 | 0 |
| [/properties/deposits](./properties-deposits.md) | `/properties/deposits` | `deposits` | 292 | 1 | 5 | 0 |
| [/properties/occupancy-report](./properties-occupancy-report.md) | `/properties/occupancy-report` | `occupancy-report` | 184 | 1 | 0 | 1 |
| [/guide/properties](./guide-properties.md) | `/guide/properties` | `properties` | 1430 | 0 | 1 | 1 |
| [/properties/guide](./properties-guide.md) | `/properties/guide` | `guide` | 1430 | 0 | 1 | 1 |
| [/properties/:id/status](./properties-byid-status.md) | `/properties/:id/status` | `status` | 196 | 0 | 3 | 0 |
| [/properties/:id](./properties-byid.md) | `/properties/:id` | `:id` | 682 | 0 | 1 | 1 |
| [/properties](./properties.md) | `/properties` | `properties` | 169 | 0 | 2 | 1 |
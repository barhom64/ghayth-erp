# وحدة `operations` — نظرة عامة

> الموجة: **—** | عدد الصفحات: **43** | أوراق منشأة: **43**

## KPIs
- مشاكل إجمالية: **0** (🔴 0 / ⚠ 0 / ℹ 0)

## الصفحات

| الصفحة | المسار | الكيان | سطور | API calls | أزرار | مشاكل |
|--------|--------|--------|------|-----------|--------|--------|
| [/obligations](./obligations.md) | `/obligations` | `obligations` | 229 | 1 | 0 | 0 |
| [/operations-center](./operations-center.md) | `/operations-center` | `operations-center` | 284 | 0 | 4 | 0 |
| [/daily-close](./daily-close.md) | `/daily-close` | `daily-close` | 218 | 0 | 1 | 0 |
| [/projects](./projects.md) | `/projects` | `projects` | 400 | 1 | 5 | 0 |
| [/projects/create](./projects-create.md) | `/projects/create` | `create` | 146 | 1 | 3 | 0 |
| [/projects/tasks](./projects-tasks.md) | `/projects/tasks` | `tasks` | 423 | 0 | 9 | 0 |
| [/projects/gantt](./projects-gantt.md) | `/projects/gantt` | `gantt` | 182 | 1 | 0 | 0 |
| [/projects/risks](./projects-risks.md) | `/projects/risks` | `risks` | 288 | 1 | 1 | 0 |
| [/projects/:id](./projects-byid.md) | `/projects/:id` | `:id` | 734 | 0 | 21 | 0 |
| [/tasks/create](./tasks-create.md) | `/tasks/create` | `create` | 203 | 1 | 3 | 0 |
| [/tasks/:id](./tasks-byid.md) | `/tasks/:id` | `:id` | 223 | 0 | 0 | 0 |
| [/tasks](./tasks.md) | `/tasks` | `tasks` | 423 | 0 | 9 | 0 |
| [/umrah](./umrah.md) | `/umrah` | `umrah` | 171 | 1 | 0 | 0 |
| [/umrah/pilgrims](./umrah-pilgrims.md) | `/umrah/pilgrims` | `pilgrims` | 146 | 0 | 0 | 0 |
| [/umrah/pilgrims/create](./umrah-pilgrims-create.md) | `/umrah/pilgrims/create` | `create` | 104 | 3 | 2 | 0 |
| [/umrah/pilgrims/:id](./umrah-pilgrims-byid.md) | `/umrah/pilgrims/:id` | `:id` | 169 | 0 | 0 | 0 |
| [/umrah/agents](./umrah-agents.md) | `/umrah/agents` | `agents` | 238 | 1 | 5 | 0 |
| [/umrah/agents/:id](./umrah-agents-byid.md) | `/umrah/agents/:id` | `:id` | 253 | 0 | 0 | 0 |
| [/umrah/seasons](./umrah-seasons.md) | `/umrah/seasons` | `seasons` | 94 | 1 | 3 | 0 |
| [/umrah/seasons/:id](./umrah-seasons-byid.md) | `/umrah/seasons/:id` | `:id` | 233 | 0 | 0 | 0 |
| [/umrah/penalties](./umrah-penalties.md) | `/umrah/penalties` | `penalties` | 243 | 1 | 2 | 0 |
| [/umrah/penalties/:id](./umrah-penalties-byid.md) | `/umrah/penalties/:id` | `:id` | 311 | 0 | 0 | 0 |
| [/umrah/invoices](./umrah-invoices.md) | `/umrah/invoices` | `invoices` | 147 | 3 | 0 | 0 |
| [/umrah/invoices/:id](./umrah-invoices-byid.md) | `/umrah/invoices/:id` | `:id` | 336 | 0 | 0 | 0 |
| [/umrah/packages](./umrah-packages.md) | `/umrah/packages` | `packages` | 244 | 2 | 5 | 0 |
| [/umrah/packages/:id](./umrah-packages-byid.md) | `/umrah/packages/:id` | `:id` | 276 | 0 | 0 | 0 |
| [/umrah/transport](./umrah-transport.md) | `/umrah/transport` | `transport` | 113 | 1 | 2 | 0 |
| [/umrah/transport/:id](./umrah-transport-byid.md) | `/umrah/transport/:id` | `:id` | 151 | 0 | 0 | 0 |
| [/umrah/import/legacy](./umrah-import-legacy.md) | `/umrah/import/legacy` | `legacy` | 264 | 2 | 1 | 0 |
| [/umrah/sub-agents](./umrah-sub-agents.md) | `/umrah/sub-agents` | `sub-agents` | 387 | 0 | 2 | 0 |
| [/umrah/sub-agents/:id](./umrah-sub-agents-byid.md) | `/umrah/sub-agents/:id` | `:id` | 202 | 0 | 2 | 0 |
| [/umrah/pricing](./umrah-pricing.md) | `/umrah/pricing` | `pricing` | 407 | 0 | 2 | 0 |
| [/umrah/commission-plans](./umrah-commission-plans.md) | `/umrah/commission-plans` | `commission-plans` | 327 | 0 | 2 | 0 |
| [/umrah/commission-plans/new](./umrah-commission-plans-new.md) | `/umrah/commission-plans/new` | `new` | 641 | 0 | 1 | 0 |
| [/umrah/commission-plans/:id/edit](./umrah-commission-plans-byid-edit.md) | `/umrah/commission-plans/:id/edit` | `edit` | 641 | 0 | 1 | 0 |
| [/umrah/violations](./umrah-violations.md) | `/umrah/violations` | `violations` | 522 | 0 | 3 | 0 |
| [/umrah/violations/create](./umrah-violations-create.md) | `/umrah/violations/create` | `create` | 316 | 1 | 2 | 0 |
| [/umrah/violations/:id](./umrah-violations-byid.md) | `/umrah/violations/:id` | `:id` | 82 | 0 | 0 | 0 |
| [/umrah/import](./umrah-import.md) | `/umrah/import` | `import` | 491 | 0 | 5 | 0 |
| [/umrah/daily-runsheet](./umrah-daily-runsheet.md) | `/umrah/daily-runsheet` | `daily-runsheet` | 177 | 1 | 1 | 0 |
| [/umrah/reconciliation](./umrah-reconciliation.md) | `/umrah/reconciliation` | `reconciliation` | 186 | 1 | 0 | 0 |
| [/umrah/groups](./umrah-groups.md) | `/umrah/groups` | `groups` | 313 | 0 | 1 | 0 |
| [/umrah/attachments](./umrah-attachments.md) | `/umrah/attachments` | `attachments` | 185 | 0 | 0 | 0 |
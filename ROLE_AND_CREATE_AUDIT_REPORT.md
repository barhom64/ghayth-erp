...
| `POST /api/rbac/jit/:id/approve` | `rbacV2.ts:1291` | `parse(req.body \|\| {})` → `zodParse(...safeParse())` ✅ (مكتشف لاحقًا في مراجعة الكود — نفس الصنف) |
| `POST /api/rbac/jit/:id/reject` | `rbacV2.ts:1379` | `parse(req.body \|\| {})` → `zodParse(...safeParse())` ✅ (مكتشف لاحقًا في مراجعة الكود — نفس الصنف) |

**التحقق بعد الإصلاح:** إعادة الفحص → `BUG-500 count: 0`؛ المسارات ترجع الآن 422. typecheck لـ api-server يمر. الناتج الخام: `/tmp/create-smoke.json`.
...
## حالة الدمج
الإصلاحات الخمسة مطبّقة ومُتحقّق منها ومدموجة على main عبر طلبَي سحب باجتياز guard:
- **PR #2669** (squash `a3c4191e`): الإصلاحات الثلاثة الأولى + محاذاة اختبار `hrContractsSmoke.test.ts` (كان يثبّت نمط `.parse` القديم) + هذا التقرير.
- **PR #2674** (squash `78e61e79`): مسارا JIT approve/reject (نفس الصنف، اكتُشِفا في مراجعة الكود بعد دمج #2669).

/**
 * فاتورة وكيل فرعي
 * POST /api/umrah/sub-agent-invoices
 */
...
export default function فاتورةوكيلفرعيScreen() {
  const c = useColors();
  ...
  const mutation = useMutation('/api/umrah/sub-agent-invoices', 'POST');

  const validate = () => {
    const e: Record<string, string> = {};
    if (!subAgentName) e['subAgentName'] = 'الوكيل الفرعي مطلوب';
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: c.bg }} contentContainerStyle={{ padding: 16, gap: 16, paddingBottom: 40 }}>
      <Stack.Screen options={{ title: 'فاتورة وكيل فرعي' }} />

      <GCard style={styles.card}>
        <GInput label="الوكيل الفرعي *" value={subAgentName} onChangeText={setSubAgentName} placeholder="اسم الوكيل الفرعي" error={errors["subAgentName"]} />

...
interface Journey { id?: number; status?: string; departureDate?: string; returnDate?: string; }

export default function SubAgentJourneyScreen() {
  const c = useColors();
  const { data, isLoading, isError, refetch } = useList<Journey>('/api/umrah/sub-agents/0/journey');
  ...
  return (
    <View style={{ flex: 1, backgroundColor: c.bg }}>
      <Stack.Screen options={{ title: 'رحلة الوكيل الفرعي' }} />
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
        {!d ? (
          ...
          <>
            ...
            <View style={{ backgroundColor: c.surface, borderRadius: 8, padding: 14, marginBottom: 12 }}>
              <Text style={{ color: c.textMuted, fontSize: 12, marginBottom: 4 }}>تاريخ العودة</Text>
              <Text style={{ color: c.text, fontSize: 14 }}>{d.returnDate ?? '—'}</Text>
            </View>
          </>
        )}
      </ScrollView>

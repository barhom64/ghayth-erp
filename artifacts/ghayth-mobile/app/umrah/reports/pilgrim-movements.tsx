...
export default function Screen() {
  const c = useColors();
  const { data, isLoading, isError, refetch } = useList<Item[]>('/api/umrah/reports/pilgrim-movements');
  ...
  return (
    <View style={{ flex: 1, backgroundColor: c.bg }}>
      ...
      <FlatList data={list} keyExtractor={(item, i) => String(item.id ?? i)}
        ...
        ListEmptyComponent={<GEmptyState icon="walk-outline" title="لا توجد بيانات" description="" />}
        renderItem={({ item }) => (

...
export default function UmrahCalendarScreen() {
  const c = useColors();
  const { data, isLoading, isError, refetch } = useList<UmrahCalendarEvent[]>('/api/umrah-calendar/calendar/events');
  const list = Array.isArray(data) ? data : [];

  if (isLoading) return <GLoadingState text="جارٍ تحميل تقويم العمرة…" />;
  if (isError) return (
  ...
  return (
    <View style={{ flex: 1, backgroundColor: c.bg }}>
      <Stack.Screen options={{ title: 'تقويم العمرة' }} />
      <FlatList
        data={list}
        ...
        renderItem={({ item }) => (
          <Pressable style={{ backgroundColor: c.surface, borderBottomWidth: 1, borderBottomColor: c.border, padding: 14 }}>
            <View style={{ flexDirection: 'row-reverse', alignItems: 'center', gap: 8, marginBottom: 4 }}>
              ...
              {item.eventType ? <Text style={{ fontSize: 11, color: c.brand }}>{item.eventType}</Text> : null}
            </View>

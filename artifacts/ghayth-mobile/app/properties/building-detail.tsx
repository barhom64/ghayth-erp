...
interface Building { id?: number; name?: string; address?: string; floors?: number; units?: number; type?: string; status?: string; }

export default function BuildingDetail() {
  const c = useColors();
  const { id } = useLocalSearchParams<{ id: string }>();
  const { data, isLoading, isError, refetch } = useList<Building>(`/api/properties/buildings/${id ?? '0'}`);
  const d = (data && !Array.isArray(data)) ? data as Building : null;
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: c.bg }} contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
      <Stack.Screen options={{ title: d.name ?? 'تفاصيل المبنى' }} />
      {row('الاسم', d.name)}
      {row('العنوان', d.address)}
      {row('النوع', d.type)}
      {row('الطوابق', d.floors)}
      {row('الوحدات', d.units)}
      {row('الحالة', d.status)}
    </ScrollView>
  );

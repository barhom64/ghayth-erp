  ...
  return (
    <View style={{ flex: 1, backgroundColor: c.bg }}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <Pressable
            onPress={() => router.push({ pathname: '/support/kb-detail' as never, params: { id: item.id } })}
          ...
          >
            <View style={{ flexDirection: 'row-reverse', alignItems: 'center', gap: 8, marginBottom: 4 }}>
              ...
              {item.isPublic ? <Text style={{ fontSize: 10, color: '#22C55E', fontWeight: '700' }}>عام</Text> : <Text style={{ fontSize: 10, color: c.textFaint }}>داخلي</Text>}
            </View>
            <View style={{ flexDirection: 'row-reverse', gap: 8 }}>
              {item.category ? <Text style={{ fontSize: 12, color: c.brand }}>{item.category}</Text> : null}
              {item.viewCount != null ? <Text style={{ fontSize: 11, color: c.textFaint }}>{item.viewCount} مشاهدة</Text> : null}

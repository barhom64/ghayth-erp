...
export default function RequestsStatsScreen() {
  const c = useColors();
  const { data, isLoading, isError } = useList<RequestsStats>('/api/requests/stats');
  const d = (data && !Array.isArray(data)) ? data as RequestsStats : null;

  if (isLoading) return <GLoadingState text="جارٍ تحميل إحصاءات الطلبات…" />;
  if (isError) return (
  ...
  return (
    <View style={{ flex: 1, backgroundColor: c.bg }}>
      <Stack.Screen options={{ title: 'إحصاءات الطلبات' }} />
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40, gap: 12 }}>
        ...
        <View style={{ flexDirection: 'row-reverse', flexWrap: 'wrap', gap: 10 }}>
          {[
            ...
            { label: 'متوسط الحل (أيام)', value: d?.avgResolutionDays ?? 0, color: c.brand },
          ].map(m => (
            <View key={m.label} style={{ backgroundColor: c.surface, borderRadius: 10, padding: 14, minWidth: '30%', flex: 1, alignItems: 'center' }}>

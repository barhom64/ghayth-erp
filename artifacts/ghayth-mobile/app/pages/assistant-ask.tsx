...
export default function Screen() {
  const c = useColors();
  const { data, isLoading, isError, refetch } = useList<Item[]>('/api/assistant');
  ...
  return (
    <View style={{ flex: 1, backgroundColor: c.bg }}>
      ...
      <FlatList data={list} keyExtractor={(item, i) => String(item.id ?? i)}
        ...
        ListEmptyComponent={<GEmptyState icon="chatbubble-ellipses-outline" title="لا توجد بيانات" description="" />}
        renderItem={({ item }) => (

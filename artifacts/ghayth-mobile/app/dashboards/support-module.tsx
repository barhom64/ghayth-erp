import React from 'react';
import { ScrollView, Text, View } from 'react-native';
import { Stack } from 'expo-router';
import { GLoadingState, GEmptyState } from '@workspace/ui-native';
import { useColors } from '@/hooks/useColors';
import { useList } from '@/hooks/useApi';

interface SupportModuleDashboard {
  totalTickets?: number;
  openTickets?: number;
  resolvedTickets?: number;
  slaBreach?: number;
  avgResolutionHours?: number;
  csatAvg?: string | number;
  pendingTickets?: number;
  escalatedTickets?: number;
  [key: string]: unknown;
}

export default function SupportModuleDashboardScreen() {
  const c = useColors();
  const { data, isLoading, isError } = useList<SupportModuleDashboard>('/api/module-dashboards/support');
  const d = (data && !Array.isArray(data)) ? data as SupportModuleDashboard : null;

  if (isLoading) return <GLoadingState text="جارٍ تحميل لوحة الدعم…" />;
  if (isError) return (
    <GEmptyState icon="alert-circle-outline" title="تعذّر التحميل" description="تحقق من الاتصال" />
  );

  const csat = parseFloat(String(d?.csatAvg ?? '0'));
  const csatColor = csat >= 4 ? '#22C55E' : csat >= 3 ? '#F59E0B' : '#EF4444';

  return (
    <View style={{ flex: 1, backgroundColor: c.bg }}>
      <Stack.Screen options={{ title: 'لوحة الدعم' }} />
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40, gap: 12 }}>
        {d?.csatAvg != null ? (
          <View style={{ backgroundColor: c.surface, borderRadius: 12, padding: 20, alignItems: 'center', borderTopWidth: 4, borderTopColor: csatColor }}>
            <Text style={{ fontSize: 48, fontWeight: '700', color: csatColor }}>{d.csatAvg}</Text>
            <Text style={{ fontSize: 14, color: c.textMuted, marginTop: 4 }}>متوسط رضا العملاء / 5</Text>
          </View>
        ) : null}
        <View style={{ flexDirection: 'row-reverse', flexWrap: 'wrap', gap: 10 }}>
          {[
            { label: 'إجمالي التذاكر', value: d?.totalTickets ?? 0, color: c.text },
            { label: 'مفتوحة', value: d?.openTickets ?? 0, color: '#F59E0B' },
            { label: 'محلولة', value: d?.resolvedTickets ?? 0, color: '#22C55E' },
            { label: 'تجاوز SLA', value: d?.slaBreach ?? 0, color: (d?.slaBreach ?? 0) > 0 ? '#EF4444' : c.text },
            { label: 'معلقة', value: d?.pendingTickets ?? 0, color: '#F59E0B' },
            { label: 'مصعّدة', value: d?.escalatedTickets ?? 0, color: '#EF4444' },
          ].map(m => (
            <View key={m.label} style={{ backgroundColor: c.surface, borderRadius: 10, padding: 14, minWidth: '45%', flex: 1, alignItems: 'center' }}>
              <Text style={{ fontSize: 22, fontWeight: '700', color: m.color }}>{m.value}</Text>
              <Text style={{ fontSize: 11, color: c.textMuted, marginTop: 2 }}>{m.label}</Text>
            </View>
          ))}
        </View>
        {d?.avgResolutionHours != null ? (
          <View style={{ backgroundColor: c.surface, borderRadius: 10, padding: 14, flexDirection: 'row-reverse', justifyContent: 'space-between' }}>
            <Text style={{ fontSize: 13, color: c.textMuted }}>متوسط وقت الحل</Text>
            <Text style={{ fontSize: 14, fontWeight: '700', color: c.text }}>{d.avgResolutionHours} ساعة</Text>
          </View>
        ) : null}
      </ScrollView>
    </View>
  );
}

...
export default function CrmContractsScreen() {
  const c = useColors();
  const router = useRouter();
  const { data, isLoading, isError, refetch } = useList<CrmContract[]>('/api/crm/contracts');
  ...
  return (
    <View style={{ flex: 1, backgroundColor: c.bg }}>
      <Stack.Screen options={{ title: 'عقود العملاء' }} />
      <FlatList
        data={list}
        ...
        renderItem={({ item }) => (
          <Pressable
            onPress={() => router.push({ pathname: '/crm/contract-detail' as never, params: { id: item.id } })}

/**
 * دفعة مقدمة من عميل
 * POST /api/finance/customer-advances
 */
...
export default function دفعةمقدمةمنعميلScreen() {
  const c = useColors();
  ...
  const mutation = useMutation('/api/finance/customer-advances', 'POST');

  const validate = () => {
    const e: Record<string, string> = {};
    if (!clientName) e['clientName'] = 'اسم العميل مطلوب';
    if (!amount) e['amount'] = 'المبلغ مطلوب';
    if (!receiptDate) e['receiptDate'] = 'تاريخ الاستلام مطلوب';
    setErrors(e);
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: c.bg }} contentContainerStyle={{ padding: 16, gap: 16, paddingBottom: 40 }}>
      <Stack.Screen options={{ title: 'دفعة مقدمة من عميل' }} />

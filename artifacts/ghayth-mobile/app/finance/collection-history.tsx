...
interface CollectionEvent { id?: number; action?: string; amount?: number; date?: string; actor?: string; }

export default function CollectionHistoryScreen() {
  const c = useColors();
  const { data, isLoading, isError, refetch } = useList<CollectionEvent[]>('/api/finance/collection/0/history');
      ...
      <FlatList data={list} keyExtractor={(item, i) => String(item.id ?? i)}
        ...
        renderItem={({ item }) => (
          <View style={{ backgroundColor: c.surface, borderBottomWidth: 1, borderBottomColor: c.border, padding: 14 }}>
            ...
            {item.actor && <Text style={{ color: c.textMuted, fontSize: 12 }}>{item.actor}</Text>}
            {item.date && (

...
export default function ArAgingScreen() {
  const c = useColors();
  const { data, isLoading, isError, refetch } = useList<ArAgingBucket[]>('/api/finance/ar-aging');
  const list = Array.isArray(data) ? data : [];

  if (isLoading) return <GLoadingState text="جارٍ تحميل تقادم الذمم المدينة…" />;
  if (isError) return (
  ...
  return (
    <View style={{ flex: 1, backgroundColor: c.bg }}>
      <Stack.Screen options={{ title: 'تقادم الذمم المدينة' }} />
      <FlatList
        data={list}
        ...
        renderItem={({ item }) => (
          <Pressable style={{ backgroundColor: c.surface, borderBottomWidth: 1, borderBottomColor: c.border, padding: 14 }}>
            <Text style={{ fontSize: 13, fontWeight: '700', color: c.text, textAlign: 'right', marginBottom: 6 }}>{item.clientName ?? '—'}</Text>

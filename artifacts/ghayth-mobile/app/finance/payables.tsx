...
export default function PayablesScreen() {
  const c = useColors();
  const { data, isLoading, isError, refetch } = useList<Payable[]>('/api/finance/payables');
  const list = Array.isArray(data) ? data : [];

  if (isLoading) return <GLoadingState text="جارٍ تحميل الذمم الدائنة…" />;
  if (isError) return (
  ...
  return (
    <View style={{ flex: 1, backgroundColor: c.bg }}>
      <Stack.Screen options={{ title: 'الذمم الدائنة' }} />
      <FlatList
        data={list}
        ...
        ListEmptyComponent={<GEmptyState icon="card-outline" title="لا توجد ذمم دائنة" description="" />}
        renderItem={({ item }) => {

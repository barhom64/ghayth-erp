/**
 * عقود الموردين
 * GET /api/finance/vendor-contracts
 */
...
export default function VendorContractsScreen() {
  ...
  const { data, isLoading, isError, refetch } = useList<VendorContract[]>('/api/finance/vendor-contracts');
  const list = Array.isArray(data) ? data : [];

  if (isLoading) return <GLoadingState text="جارٍ تحميل عقود الموردين…" />;
  if (isError) return (
  ...
  return (
    <View style={{ flex: 1, backgroundColor: c.bg }}>
      <Stack.Screen options={{ title: 'عقود الموردين' }} />
      <FlatList
        data={list}
        ...
        renderItem={({ item }) => (
          <Pressable
            onPress={() => router.push({ pathname: '/finance/vendor-contract-detail' as never, params: { id: item.id } })}
          ...
          >
            ...
            {item.contractType ? <Text style={{ fontSize: 12, color: c.textMuted, textAlign: 'right' }}>{item.contractType}</Text> : null}

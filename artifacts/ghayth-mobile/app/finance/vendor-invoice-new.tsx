/**
 * إنشاء فاتورة مورد جديدة — POST /api/finance/vendor-invoices
 */
...
export default function VendorInvoiceNewScreen() {
  const c = useColors();
  const router = useRouter();
  const qc = useQueryClient();
  const { vendorId: vendorIdParam, purchaseOrderId: poIdParam } = useLocalSearchParams<{
    vendorId?: string;
  ...
  const mutation = useMutation('/api/finance/vendor-invoices', 'POST');
  ...
  const totals = lines.reduce(
    (acc, l) => { const t = calcLine(l); return { subtotal: acc.subtotal + t.subtotal, vat: acc.vat + t.vat, total: acc.total + t.total }; },
  ...
  const onSubmit = async () => {
    ...
    try {
      const body: Record<string, unknown> = {
        ...
        lines: lines.map(l => {
          const t = calcLine(l);
          return { description: l.description, quantity: Number(l.quantity) || 1, unitPrice: Number(l.unitPrice) || 0, vatRate: Number(l.vatRate) || 0, subtotal: t.subtotal, vatAmount: t.vat, total: t.total };
        }),
      ...
      if (poIdParam) body.purchaseOrderId = Number(poIdParam);
      if (notes) body.notes = notes;

      await mutation.mutateAsync(body as never);
      qc.invalidateQueries({ queryKey: ['/api/finance/vendor-invoices'] });
      Alert.alert('تم', 'تم إدراج فاتورة المورد بنجاح', [
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: c.bg }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <Stack.Screen options={{ title: 'فاتورة مورد جديدة' }} />
      <ScrollView contentContainerStyle={styles.container}>
        <GCard>
          ...
          <GInput label="رقم الفاتورة" value={invoiceNumber} onChangeText={setInvoiceNumber} placeholder="رقم الفاتورة من المورد" />
          <DateInput label="تاريخ الفاتورة" value={issueDate} onChange={setIssueDate} />
        ...
        {lines.map((line, i) => (
          <GCard key={i} style={{ gap: 8 }}>
            ...
            <GInput label="الوصف *" value={line.description} onChangeText={v => updateLine(i, 'description', v)} placeholder="اسم المادة أو الخدمة" />
        ...
        <GCard>
          ...
          <GButton title="حفظ الفاتورة" icon="save-outline" onPress={onSubmit} loading={mutation.isPending} style={{ marginTop: 8 }} />
        </GCard>
      </ScrollView>

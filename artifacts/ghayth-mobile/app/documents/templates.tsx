...
export default function DocumentTemplatesScreen() {
  const c = useColors();
  const { data, isLoading, isError, refetch } = useList<DocTemplate[]>('/api/documents/templates');
  const list = Array.isArray(data) ? data : [];

  if (isLoading) return <GLoadingState text="جارٍ تحميل قوالب المستندات…" />;
  if (isError) return (
  ...
  return (
    <View style={{ flex: 1, backgroundColor: c.bg }}>
      <Stack.Screen options={{ title: 'قوالب المستندات' }} />
      <FlatList
        data={list}
        ...
        renderItem={({ item }) => (
          <View style={{ backgroundColor: c.surface, borderBottomWidth: 1, borderBottomColor: c.border, padding: 14, flexDirection: 'row-reverse', justifyContent: 'space-between', alignItems: 'center' }}>
            <View style={{ flex: 1 }}>
              ...
              {item.language ? <Text style={{ fontSize: 11, color: c.textMuted, marginTop: 2 }}>{item.language}</Text> : null}
            </View>
            <View style={{ alignItems: 'flex-end' }}>
              {item.usageCount != null ? <Text style={{ fontSize: 12, color: c.textMuted }}>{item.usageCount} استخدام</Text> : null}

...
interface Session { id?: number; date?: string; venue?: string; notes?: string; outcome?: string; }

export default function LegalSessionDetailScreen() {
  const c = useColors();
  const { data, isLoading, isError, refetch } = useList<Session>('/api/legal/sessions/0');
  const d = (data && !Array.isArray(data)) ? data as Session : null;
  if (isLoading) return <GLoadingState text="جارٍ تحميل…" />;
  if (isError) return (
    <GEmptyState icon="alert-circle-outline" title="تعذّر التحميل" description="تحقق من الاتصال"
      actionLabel="إعادة المحاولة" onAction={refetch} />
  );
  if (!d) return <GEmptyState icon="alert" title="لا توجد بيانات" description="" />;
  return (
    <ScrollView style={{ flex: 1, backgroundColor: c.bg }}>
      <Stack.Screen options={{ title: 'تفاصيل الجلسة' }} />
      <View style={{ padding: 16, gap: 12 }}>
        ...
        <View style={{ backgroundColor: c.surface, borderRadius: 8, padding: 14 }}>
          <Text style={{ color: c.textMuted, fontSize: 12 }}>المكان</Text>
          <Text style={{ color: c.text, fontSize: 15, marginTop: 4 }}>{d.venue ?? '—'}</Text>
        </View>
        <View style={{ backgroundColor: c.surface, borderRadius: 8, padding: 14 }}>

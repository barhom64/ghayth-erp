...
interface PrintTemplate { id?: number; name?: string; entityType?: string; format?: string; }
      ...
      <FlatList data={list} keyExtractor={(item, i) => String(item.id ?? i)}
        ...
        renderItem={({ item }) => (
          <View style={{ backgroundColor: c.surface, borderBottomWidth: 1, borderBottomColor: c.border, padding: 14 }}>
            ...
            <Text style={{ color: c.textMuted, fontSize: 12, marginTop: 4 }}>{item.entityType ?? ''} · {item.format ?? ''}</Text>
          </View>
        )}
      />
    </View>

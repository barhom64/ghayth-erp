...
interface PrintQueueItem { id?: number; status?: string; priority?: number; template?: string; createdAt?: string; }

export default function PrintQueueDetail() {
  const c = useColors();
  const { id } = useLocalSearchParams<{ id: string }>();
  const { data, isLoading, isError, refetch } = useList<PrintQueueItem>(`/api/print/queue/${id ?? '0'}`);
  const d = (data && !Array.isArray(data)) ? data as PrintQueueItem : null;
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: c.bg }} contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
      <Stack.Screen options={{ title: 'تفاصيل الطباعة' }} />
      {row('الحالة', d.status)}
      {row('الأولوية', d.priority)}
      {row('القالب', d.template)}

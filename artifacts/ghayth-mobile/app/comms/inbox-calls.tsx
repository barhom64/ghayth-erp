...
export default function InboxCallsScreen() {
  const c = useColors();
  const { data, isLoading, isError, refetch } = useList<InboxCall[]>('/api/inbox/calls');
  const list = Array.isArray(data) ? data : [];

  if (isLoading) return <GLoadingState text="جارٍ تحميل المكالمات…" />;
  if (isError) return (
        ...
        renderItem={({ item }) => (
          <Pressable style={{ backgroundColor: c.surface, borderBottomWidth: 1, borderBottomColor: c.border, padding: 14 }}>
            <View style={{ flexDirection: 'row-reverse', alignItems: 'center', gap: 8, marginBottom: 4 }}>
              <Text style={{ fontSize: 13, fontWeight: '700', color: c.text, flex: 1, textAlign: 'right' }}>{item.callerNumber ?? '—'}</Text>
              <GStatusBadge status={item.status ?? ''} />
            </View>
            <View style={{ flexDirection: 'row-reverse', gap: 8 }}>
              {item.direction ? <Text style={{ fontSize: 12, color: item.direction === 'inbound' ? '#22C55E' : c.brand }}>{item.direction === 'inbound' ? 'واردة' : 'صادرة'}</Text> : null}

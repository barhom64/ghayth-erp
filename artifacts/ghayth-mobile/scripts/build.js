...
function getDeploymentDomain() {
  // 1. Explicit override — set this for production mobile builds so the shipped
  //    bundle ALWAYS targets the stable backend (same as the web), even when the
  //    build runs on Replit where REPLIT_DEV_DOMAIN would otherwise win and
  //    rotate. This is what unifies mobile + web onto one backend/DB.
  ...
  // Fallback: the stable production backend. Builds run outside Replit (CI/VPS)
  // have no REPLIT_* domain vars, so default to the canonical operational domain
  // instead of failing.
  console.log(

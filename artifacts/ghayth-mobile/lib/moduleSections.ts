...
/**
 * One field in an action's pre-fire prompt sheet. Unlike `FormFieldDef` (used by
 * the create/edit form engine), a prompt field is collected inline on the detail
 * screen — in a small sheet — right before a server action fires, so operations
 * that need dynamic input (a typed reason/amount, a renewal date, a set of
 * pilgrims to split off) can run without a dedicated screen. The server stays
 * the final validator; these checks are only light client-side feedback.
 */
export type ActionPromptFieldType =
  | "text"
  | "textarea"
  | "number"
  | "currency"
...
export interface ActionPromptField {
  ...
  name: string;
  label: string;
  type: ActionPromptFieldType;
  required?: boolean;
  placeholder?: string;
  /** Static options for `select`. */
  options?: FormFieldOption[];
  /**
   * `reference` only: GET list endpoint whose rows become options. May be a
   * function of the current record id so the list can be scoped (e.g. the
   * pilgrims of *this* group).
   */
  refEndpoint?: string | ((id: string | number) => string);
  /** `reference` only: first present field becomes the option label. */
  refLabelFields?: string[];
  /** `reference` only: row field used as the option value (default `id`). */
  refValueField?: string;
  /**
   * Collect multiple values, sent to the API as an array. Only meaningful for
   * `reference` / `select`. Used by bulk endpoints (split pilgrims, merge
   * source groups).
   */
  multi?: boolean;
}
...
export interface SectionAction {
  ...
  /**
   * Prompt fields collected via a small sheet *before* the call fires; the
   * collected values are merged into the request body. Used for operations that
   * require dynamic input (typed reason/amount, renewal date, pilgrim/group
   * selection). When set, the sheet replaces the simple `confirm` alert.
   */
  prompts?: ActionPromptField[];
  /**
   * Inject the current record's id into the request body under this key before
   * firing — for bulk endpoints whose path does not carry the id (e.g.
   * `/penalties/waive-bulk`, `/groups/merge`). `idBodyAsArray` wraps it in a
   * single-element array.
   */
  idBodyField?: string;
  idBodyAsArray?: boolean;
    ...
    sections: [
      {
        ...
        write: {
          ...
          createFields: [
            ...
            { name: "basicSalary", label: "الراتب الأساسي", type: "currency" },
      ...
      },
      {
        ...
        write: {
          ...
          createFields: [
            {
              name: "leaveType", label: "نوع الإجازة", type: "select", required: true,
              options: [
  ...
  umrah: {
    ...
    sections: [
      ...
      },
      {
        ...
        write: {
          moduleKey: "umrah",
          detailPath: (id) => `/api/umrah/groups/${id}`,
          createFields: [
            {
          ...
          actions: [
            {
              key: "split", label: "تقسيم المجموعة", icon: "git-branch-outline", method: "POST",
              path: (id) => `/api/umrah/groups/${id}/split`,
              confirm: "اختر المعتمرين المراد نقلهم إلى مجموعة جديدة.",
              successText: "تم تقسيم المجموعة",
              prompts: [
                {
                  name: "pilgrimIds", label: "المعتمرون المراد نقلهم", type: "reference", required: true, multi: true,
                  refEndpoint: (id) => `/api/umrah/pilgrims?groupId=${id}`,
                  refLabelFields: ["fullName", "name"], refValueField: "id",
                },
                { name: "newGroupName", label: "اسم المجموعة الجديدة", type: "text" },
                { name: "newNuskGroupNumber", label: "رقم مجموعة نسك الجديدة", type: "text" },
              ],
            },
            {
              key: "merge", label: "دمج مجموعات", icon: "git-merge-outline", method: "POST",
              path: () => `/api/umrah/groups/merge`,
              idBodyField: "targetGroupId",
              confirm: "اختر المجموعات المصدر التي سيتم دمج معتمريها في هذه المجموعة.",
              successText: "تم دمج المجموعات",
              prompts: [
                {
                  name: "sourceGroupIds", label: "المجموعات المصدر", type: "reference", required: true, multi: true,
                  refEndpoint: "/api/umrah/groups", refLabelFields: ["name", "groupNumber", "nuskGroupNumber"], refValueField: "id",
                },
              ],
            },
          ],
      ...
      { key: "violations", label: "المخالفات", icon: "warning-outline", endpoint: "/api/umrah/violations", titleFields: ["type"], subtitleFields: ["mutamerName", "agentName"], statusField: "status", amountFields: ["penaltyAmount"], dateFields: ["detectedAt"] },
      {
        ...
        write: {
          moduleKey: "umrah",
          detailPath: (id) => `/api/umrah/penalties/${id}`,
          actions: [
            {
              key: "waive", label: "إعفاء الغرامة", icon: "remove-circle-outline", method: "POST",
              path: () => `/api/umrah/penalties/waive-bulk`,
              idBodyField: "penaltyIds", idBodyAsArray: true,
              confirm: "سيتم إعفاء هذه الغرامة مع عكس القيد المحاسبي. متابعة؟",
              successText: "تم إعفاء الغرامة", tone: "danger",
              showWhenStatus: ["pending", "invoiced"],
              prompts: [
                { name: "reason", label: "سبب الإعفاء", type: "textarea", required: true, placeholder: "اكتب سبب الإعفاء" },
              ],
            },
          ],
        },
    ...
    sections: [
      ...
      },
      {
        ...
        write: {
          ...
          actions: [
            {
              key: "renew", label: "تجديد العقد", icon: "refresh-outline", method: "POST",
              path: (id) => `/api/properties/contracts/${id}/renew`,
              confirm: "أدخل مدة التجديد وتاريخ بدايته.",
              successText: "تم تجديد العقد",
              showWhenStatus: ["active", "expired"],
              prompts: [
                { name: "renewalPeriodMonths", label: "مدة التجديد (أشهر)", type: "number", placeholder: "12" },
                { name: "newStartDate", label: "تاريخ بداية التجديد", type: "date" },
                { name: "monthlyRent", label: "الإيجار الشهري الجديد", type: "currency" },
              ],
            },
      ...
      },
      {
        ...
        write: {
          ...
          actions: [
            {
              key: "renew", label: "تجديد العقد", icon: "refresh-outline", method: "POST",
              path: (id) => `/api/legal/contracts/${id}/renew`,
              confirm: "أدخل تاريخ نهاية التجديد.",
              successText: "تم تجديد العقد",
              showWhenStatus: ["active", "expired"],
              prompts: [
                { name: "newEndDate", label: "تاريخ نهاية التجديد", type: "date", required: true },
                { name: "newValue", label: "قيمة العقد بعد التجديد", type: "currency" },
                { name: "notes", label: "ملاحظات", type: "textarea" },
              ],
            },

...
invoicesRouter.post("/invoices/:id/debit-memo", authorize({ feature: "finance.invoices", action: "create" }), async (req, res) => {
  try {
    ...
    ({ journalId } = await financialEngine.postJournalEntry({
      ...
      lines: [
        { accountCode: arCode, debit: chargeAmount, credit: 0, clientId: invoice.clientId as number },
        { accountCode: revenueCode, debit: 0, credit: net, clientId: invoice.clientId as number },
        ...(vat > 0 ? [{ accountCode: vatPayableCode, debit: 0, credit: vat, clientId: invoice.clientId as number }] : []),
      ],
  ...
  try {
    ...
    for (const r of rows) {
      ...
      if (lastStage >= stg.stage && r.lastDunningAt) {
        const lastAt = new Date(r.lastDunningAt as string);
...
invoicesRouter.get("/tax/declarations", authorize({ feature: "finance.zatca", action: "list" }), async (req, res) => {
  try {
    ...
    for (let m = 1; m <= 12; m++) {
      ...
      const [stats] = await rawQuery<Record<string, unknown>>(`SELECT COALESCE(SUM("vatAmount"), 0) AS "outputVat", COUNT(*) AS "invoiceCount" FROM invoices WHERE "companyId" = $1 AND to_char("createdAt", 'YYYY-MM') = $2 AND "deletedAt" IS NULL`, [scope.companyId, period]);

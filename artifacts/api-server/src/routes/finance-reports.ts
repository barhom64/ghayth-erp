...
reportsRouter.get("/reports/trial-balance", authorize({ feature: "finance.reports", action: "list" }), async (req, res) => {
  try {
    ...
    for (const r of rows) {
      ...
      if (!byType[rt]) byType[rt] = { totalDebit: 0, totalCredit: 0, balance: 0 };
      byType[rt].totalDebit += Number(r.totalDebit);
      byType[rt].totalCredit += Number(r.totalCredit);
      byType[rt].balance += Number(r.balance);
    }
  ...
  try {
    ...
    if (entityType === "employee") {
      ...
      for (const m of journalRows) {
        const t = String(m.movementType || "other");
...
reportsRouter.get("/reports/customer-statement/:clientId", authorize({ feature: "finance.reports", action: "list" }), async (req, res) => {
  try {
    ...
    for (const inv of openInvoices) {
      const due = inv.dueDate ? new Date(inv.dueDate as string).getTime()
        : new Date(inv.createdAt as string).getTime() + 30 * 86400000;
...
reportsRouter.get("/reports/vendor-statement/:supplierId", authorize({ feature: "finance.reports", action: "list" }), async (req, res) => {
  try {
    ...
    for (const po of openPos) {
      const due = po.expectedDelivery ? new Date(po.expectedDelivery as string).getTime()
        : new Date(po.createdAt as string).getTime() + 30 * 86400000;
      const daysOverdue = Math.floor((asOfMs - due) / 86400000);

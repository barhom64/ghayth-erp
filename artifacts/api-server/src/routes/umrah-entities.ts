...
const simulateCommissionSchema = z.object({
  ...
  // §5 of #1870 — what-if overrides. Were sent by the FE simulator
  // form for months and silently ignored on the server (engine read
  // actuals from the DB only). Each field falls back to the actual
  // when undefined, so the existing month/year-only callers keep
  // working unchanged.
...
// Ad-hoc simulation against an unsaved plan — same shape as the
// persisted-plan endpoint but takes the plan + tiers inline so the
// editor can drive the simulator in create mode (before the operator
// has clicked "save"). Only the columns the engine actually reads
// are required.
...
// Simulate against an unsaved plan — for the editor's create-mode
// simulator tab. Mirrors the persisted-plan endpoint but takes the
// plan + tiers in the request body. No DB writes; no audit row (it's
// what-if math, not a preview of a real plan).
router.post("/commission-plans/simulate", authorize({ feature: "umrah", action: "list" }), async (req, res): Promise<void> => {
  try {
    ...
    const scope = req.scope!;
    // The engine's compute() reads companyId off the plan for its
    // SQL stats queries; force the request scope's tenant so a
    // cross-tenant id can't be slipped in via the body.

...
router.get("/reports/compliance", authorize({ feature: "umrah", action: "list" }), async (req, res) => {
  try {
    ...
    const [
      ...
      failedRow, missingApRow, orphanRow,
    ] = await Promise.all([
      ...
      // §3 extension — legacy orphans (Owner review point 3.ب of #1870).
      // Pilgrims with ANY NULL FK (agent OR group OR sub-agent) — catches
      // pre-#1867 doImport damage that umrah_import_changes can't see.
      rawQuery<{ c: string }>(
        `SELECT COUNT(*)::text AS c
           FROM umrah_pilgrims p
          WHERE p."companyId" = $1 AND p."deletedAt" IS NULL

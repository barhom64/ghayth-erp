...
router.post("/users", authorize({ feature: "admin", action: "update" }), async (req, res) => {
  try {
    ...
    if (isAutoGenerated) {
      // dlpExempt: credentials send. The temp password is short
      // alphanumeric — a DLP regex matching National-ID or IBAN
      // patterns would falsely redact it and lock the user out.
      // Logged in the audit row so the exemption is traceable.
      void sendMessage({
        channel: "email",
        recipient: email,
        recipientName: email,
        ...
        userId: scope.userId,
        relatedType: "user",
        relatedId: r.insertId,
        dlpExempt: true,
        templateKey: "user.credentials.welcome",
...
router.post("/user-roles", authorize({ feature: "admin", action: "update" }), async (req, res) => {
  try {
    ...
    // rbac_user_roles, and active employee_assignments). Audit-only before; a
    // hard block now.
    const existingRoles = await getActiveRoleKeysForUser(userId, scope.companyId);
    const sodConflict = findSeparationOfDutiesConflict(existingRoles, def.roleKey);
    if (sodConflict) {

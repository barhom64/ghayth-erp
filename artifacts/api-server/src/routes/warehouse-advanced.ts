...
router.post("/lots", authorize({ feature: "warehouse.inventory", action: "create" }), async (req, res) => {
  try {
    ...
    const [row] = await rawQuery<Record<string, unknown>>(
      `SELECT l.*, l."qualityControlStatus" AS "qcStatus" FROM warehouse_stock_lots l WHERE l.id=$1`, [insertId]
    );
...
router.post("/serials", authorize({ feature: "warehouse.inventory", action: "create" }), async (req, res) => {
  try {
    ...
    const [row] = await rawQuery<Record<string, unknown>>(`SELECT * FROM warehouse_stock_serials WHERE id=$1`, [insertId]);
    res.status(201).json(row);

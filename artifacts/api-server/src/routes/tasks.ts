...
async function resolveEntityName(type: string, id: number, companyId: number): Promise<string | null> {
  try {
    const queryMap: Record<string, string> = {
      property_unit: `SELECT "unitNumber" AS name FROM property_units WHERE id=$1 AND "companyId"=$2`,
      vehicle: `SELECT COALESCE("plateNumber", make || ' ' || model) AS name FROM vehicles WHERE id=$1 AND "companyId"=$2`,
      client: `SELECT name FROM clients WHERE id=$1 AND "companyId"=$2`,
      project: `SELECT name FROM projects WHERE id=$1 AND "companyId"=$2`,
      contract: `SELECT COALESCE(ref, 'عقد #' || id) AS name FROM contracts WHERE id=$1 AND "companyId"=$2`,
      legal_case: `SELECT COALESCE(title, "caseNumber", 'قضية #' || id) AS name FROM legal_cases WHERE id=$1 AND "companyId"=$2`,
      maintenance_request: `SELECT COALESCE(description, category, 'طلب #' || id) AS name FROM maintenance_requests WHERE id=$1 AND "companyId"=$2`,
    };
    const q = queryMap[type];
    if (!q) return null;
    const rows = await rawQuery<any>(q, [id, companyId]);
  ...
  try {
    ...
    const tasks = await rawQuery<any>(
      `SELECT t.id, t.title, t.description, t.status, t.priority, t.type,
              ...
              CASE
                WHEN t."linkedEntityType" = 'property_unit' THEN (SELECT pu."unitNumber" FROM property_units pu WHERE pu.id = t."linkedEntityId")
                WHEN t."linkedEntityType" = 'vehicle' THEN (SELECT v."plateNumber" FROM fleet_vehicles v WHERE v.id = t."linkedEntityId")
                WHEN t."linkedEntityType" = 'client' THEN (SELECT cl.name FROM clients cl WHERE cl.id = t."linkedEntityId")
                WHEN t."linkedEntityType" = 'maintenance_request' THEN (SELECT COALESCE(mr.category, mr.description) FROM maintenance_requests mr WHERE mr.id = t."linkedEntityId")
                WHEN t."linkedEntityType" = 'project' THEN (SELECT p.name FROM projects p WHERE p.id = t."linkedEntityId")
                WHEN t."linkedEntityType" = 'contract' THEN (SELECT rc."tenantName" FROM rental_contracts rc WHERE rc.id = t."linkedEntityId")
                WHEN t."linkedEntityType" = 'legal_case' THEN (SELECT lc."caseNumber" FROM legal_cases lc WHERE lc.id = t."linkedEntityId")
                ELSE NULL
              END AS "linkedEntityName"
       FROM tasks t
    ...
    for (const task of tasks) {
      if (task.linkedEntityType && task.linkedEntityId) {
        task.linkedEntityName = await resolveEntityName(task.linkedEntityType, task.linkedEntityId, scope.companyId);
      }
    }

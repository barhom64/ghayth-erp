...
router.get("/accounting-mappings/:operationType", async (req, res) => {
  try {
    const scope = req.scope!;
...
router.put("/accounting-mappings/:operationType", async (req, res) => {
  try {
    const scope = req.scope!;
...
router.post("/accounting-mappings/batch", async (req, res) => {
  try {
    const scope = req.scope!;
    if (!requireFinance(scope, res)) return;
...
router.get("/accounting-mappings/:operationType/validate", async (req, res) => {
  try {
    const scope = req.scope!;
...
router.get("/journal-templates", async (req, res) => {
  try {
    const scope = req.scope!;
...
router.post("/journal-templates", async (req, res) => {
  try {
    const scope = req.scope!;
    if (!requireFinance(scope, res)) return;
...
router.put("/journal-templates/:id", async (req, res) => {
  try {
    const scope = req.scope!;
    if (!requireFinance(scope, res)) return;
    const { id } = req.params;
    ...
    const [existing] = await rawQuery<any>(
      `SELECT id FROM journal_entry_templates WHERE id = $1 AND "companyId" = $2`,
...
router.delete("/journal-templates/:id", async (req, res) => {
  try {
    const scope = req.scope!;
    if (!requireFinance(scope, res)) return;
    const [existing] = await rawQuery<any>(
...
router.get("/subsidiary-accounts/entity/:entityType/:entityId", async (req, res) => {
  try {
    const scope = req.scope!;
...
router.post("/subsidiary-accounts", async (req, res) => {
  try {
    const scope = req.scope!;
    if (!requireFinance(scope, res)) return;
...
router.delete("/subsidiary-accounts/:id", async (req, res) => {
  try {
    const scope = req.scope!;
    if (!requireFinance(scope, res)) return;
    await rawExecute(
...
export async function createSubsidiaryAccountsForEntity(
  companyId: number,
  entityType: "employee" | "client" | "vendor" | "vehicle" | "property",
  entityId: number,
  entityName: string
): Promise<void> {
  try {

...
router.get("/contracts", authorize({ feature: "legal.contracts", action: "list" }), async (req, res) => {
  try {
    ...
    // #2713 (تعميم) — سلة المحذوفات: deleted=true يعرض العقود المحذوفة فقط.
...
router.get("/contracts/renewal-alerts", authorize({ feature: "legal.contracts", action: "list" }), async (req, res) => {
  try {
    ...
    const alerts90 = await rawQuery<Record<string, unknown>>(
    ...
    const alerts30 = await rawQuery<Record<string, unknown>>(
      `SELECT id, title, "partyName", "endDate", ("endDate"::date - CURRENT_DATE) AS "daysLeft"
       FROM legal_contracts WHERE "companyId"=$1 AND status='active' AND "deletedAt" IS NULL
       AND "endDate" BETWEEN CURRENT_DATE + INTERVAL '15 days' AND CURRENT_DATE + INTERVAL '30 days' LIMIT 500`,
      [cid]
    );
    const alerts14 = await rawQuery<Record<string, unknown>>(
      `SELECT id, title, "partyName", "endDate", ("endDate"::date - CURRENT_DATE) AS "daysLeft"
       FROM legal_contracts WHERE "companyId"=$1 AND status='active' AND "deletedAt" IS NULL
...
router.patch("/contracts/:id", authorize({ feature: "legal.contracts", action: "update" }), async (req, res) => {
  try {
    ...
    if (b.status !== undefined && b.status !== existing.status) {
      ...
      const allowed = LEGAL_CONTRACT_TRANSITIONS[existing.status as string] ?? [];
    ...
    if (effectiveStart && effectiveEnd && new Date(effectiveEnd as string | Date) <= new Date(effectiveStart as string | Date)) {
      throw new ValidationError(
        "تاريخ نهاية العقد يجب أن يكون بعد تاريخ البداية",
        { field: "endDate", fix: "تأكد من أن تاريخ النهاية أحدث من تاريخ البداية" }
      );
    ...
    const [row] = await rawQuery<Record<string, unknown>>(`SELECT * FROM legal_contracts WHERE id=$1 AND "companyId"=$2 AND "deletedAt" IS NULL`, [id, scope.companyId]);
...
// #2713 (تعميم) — استرجاع عقد محذوف ناعمًا (سلة المحذوفات). صلاحية حذف + Audit.
router.post("/contracts/:id/restore", authorize({ feature: "legal.contracts", action: "delete", resource: { table: "legal_contracts", idParam: "id" } }), async (req, res) => {
  try {
    const scope = req.scope!;
    const id = parseId(req.params.id, "id");
    const { affectedRows } = await rawExecute(`UPDATE legal_contracts SET "deletedAt"=NULL WHERE id=$1 AND "companyId"=$2 AND "deletedAt" IS NOT NULL`, [id, scope.companyId]);
    if (!affectedRows) throw new NotFoundError("لا يوجد عقد محذوف بهذا المعرّف");
    createAuditLog({ companyId: scope.companyId, branchId: scope.branchId, userId: scope.userId, action: "restore", entity: "legal_contracts", entityId: id }).catch((e) => logger.error(e, "legal background task failed"));
    emitEvent({ companyId: scope.companyId, branchId: scope.branchId, userId: scope.userId, action: "legal.contract.restored", entity: "legal_contracts", entityId: id }).catch((e) => logger.error(e, "legal background task failed"));
    res.json({ message: "تم استرجاع العقد" });
  } catch (err) { handleRouteError(err, res, "Restore contract error:"); }
});
...
router.post("/contracts/:id/renew", authorize({ feature: "legal.contracts", action: "create" }), async (req, res) => {
  try {
    ...
    const [current] = await rawQuery<Record<string, unknown>>(
    ...
    if (new Date(newEndDate) <= new Date(current.endDate as string | Date)) {
      throw new ValidationError(
    ...
    createAuditLog({
      ...
      after: { newEndDate, newValue: newValue ?? current.value, renewalCount: (Number(current.renewalCount) || 0) + 1 },
    }).catch((e) => logger.error(e, "legal background task failed"));
    emitEvent({
      companyId: scope.companyId,
      branchId: scope.branchId,
      ...
      entity: "legal_contracts",
      entityId: id,
      details: JSON.stringify({ newEndDate, newValue, renewalCount: (Number(current.renewalCount) || 0) + 1 }),
    }).catch((e) => logger.error(e, "legal background task failed"));
...
router.post("/cases", authorize({ feature: "legal.cases", action: "create" }), async (req, res) => {
  try {
    ...
    const [row] = await rawQuery<Record<string, unknown>>(`SELECT * FROM legal_cases WHERE id=$1 AND "companyId"=$2 AND "deletedAt" IS NULL`, [insertId, scope.companyId]);
    res.status(201).json(row);
...
router.patch("/cases/:id", authorize({ feature: "legal.cases", action: "update" }), async (req, res) => {
  try {
    const scope = req.scope!;
    const id = parseId(req.params.id, "id");
    const [existing] = await rawQuery<Record<string, unknown>>(`SELECT * FROM legal_cases WHERE id=$1 AND "companyId"=$2 AND "deletedAt" IS NULL`, [id, scope.companyId]);
    ...
    if (b.status !== undefined && b.status !== existing.status) {
      ...
      const allowed = VALID_CASE_TRANSITIONS[existing.status as string] || [];
      if (!allowed.includes(b.status)) {
        throw new ConflictError(
    ...
    if (!row) throw new NotFoundError("القضية غير موجودة");
    res.json({ ...row, allowedTransitions: VALID_CASE_TRANSITIONS[row.status as string] || [] });
...
router.delete("/cases/:id", authorize({ feature: "legal.cases", action: "delete", resource: { table: "legal_cases", idParam: "id" } }), async (req, res) => {
  try {
    ...
    if (!existing) throw new NotFoundError("القضية غير موجودة");
    if (["open", "in_progress", "judgment", "execution"].includes(existing.status as string)) {
      throw new ConflictError(
...
router.post("/cases/:caseId/sessions", authorize({ feature: "legal.cases", action: "create" }), async (req, res) => {
  try {
    ...
    const [legalCase] = await rawQuery<Record<string, unknown>>(`SELECT * FROM legal_cases WHERE id=$1 AND "companyId"=$2 AND "deletedAt" IS NULL`, [caseId, scope.companyId]);
    ...
    if (legalCase.lawyerName) {
      try {
        ...
        if (lawyerEmp?.assignmentId) {
          createNotification({
            companyId: scope.companyId,
            assignmentId: lawyerEmp.assignmentId as number,
            type: "legal_session",
    ...
    if (b.hoursSpent && b.hourlyRate) {
      ...
      const [vatSetting] = await rawQuery<Record<string, unknown>>(`SELECT value FROM system_settings WHERE "companyId" = $1 AND key = 'vat_rate' LIMIT 1`, [scope.companyId]);
      ...
      try {
        const glResult = await legalEngine.postLegalSessionFeeGL(
          ...
          { id: insertId, caseTitle: legalCase.title as string, sessionDate: b.sessionDate, billingAmount, vatAmount }
        );
    ...
    const [row] = await rawQuery<Record<string, unknown>>(`SELECT * FROM legal_sessions WHERE id=$1 AND "deletedAt" IS NULL`, [insertId]);
...
router.get("/stats", authorize({ feature: "legal.cases", action: "list" }), async (req, res) => {
  try {
    ...
    const [[contracts], [cases], [expiring], [sessions], [contingent]] = await Promise.all([
      rawQuery<Record<string, unknown>>(`SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE status='active') as active FROM legal_contracts WHERE "companyId"=$1 AND "deletedAt" IS NULL`, [cid]),
      rawQuery<Record<string, unknown>>(`SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE status='open') as open, COUNT(*) FILTER (WHERE status='in_progress') as "inProgress" FROM legal_cases WHERE "companyId"=$1 AND "deletedAt" IS NULL`, [cid]),
      rawQuery<Record<string, unknown>>(`SELECT COUNT(*) as count FROM legal_contracts WHERE "companyId"=$1 AND "endDate" BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '30 days' AND status='active' AND "deletedAt" IS NULL`, [cid]),
...
router.post("/cases/:caseId/judgments", authorize({ feature: "legal.cases", action: "create" }), async (req, res) => {
  try {
    ...
    const [lc] = await rawQuery<Record<string, unknown>>(`SELECT * FROM legal_cases WHERE id=$1 AND "companyId"=$2 AND "deletedAt" IS NULL`, [caseId, scope.companyId]);
    if (!lc) throw new NotFoundError("القضية غير موجودة");
    ...
    const [row] = await rawQuery<Record<string, unknown>>(`SELECT * FROM legal_judgments WHERE id=$1 AND "companyId"=$2`, [insertId, scope.companyId]);
    res.status(201).json(row);
...
router.patch("/cases/:caseId/judgments/:id", authorize({ feature: "legal.cases", action: "update" }), async (req, res) => {
  try {
    ...
    const [existingJ] = await rawQuery<Record<string, unknown>>(
...
router.get("/judgments/financial-report", authorize({ feature: "legal.cases", action: "list" }), async (req, res) => {
  try {
    ...
    const [contingent] = await rawQuery<Record<string, unknown>>(
...
router.get("/financial-report", authorize({ feature: "legal.cases", action: "list" }), async (req, res) => {
  try {
    const scope = req.scope!;
    const cases = await rawQuery<Record<string, unknown>>(

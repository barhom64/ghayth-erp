...
router.patch("/contracts/:id", authorize({ feature: "legal.contracts", action: "update" }), async (req, res) => {
  try {
    ...
    const effectiveStart = b.startDate || (existing.startDate as string | undefined);
    const effectiveEnd = b.endDate || (existing.endDate as string | undefined);
...
router.post("/contracts/:id/renew", authorize({ feature: "legal.contracts", action: "create" }), async (req, res) => {
  try {
    ...
    if (new Date(newEndDate) <= new Date(current.endDate as string)) {
      throw new ValidationError(
...
router.get("/stats", authorize({ feature: "legal.cases", action: "list" }), async (req, res) => {
  try {
    const scope = req.scope!;
    const cid = scope.companyId;
    const [contracts] = await rawQuery<Record<string, unknown>>(`SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE status='active') as active FROM legal_contracts WHERE "companyId"=$1 AND "deletedAt" IS NULL`, [cid]);
    const [cases] = await rawQuery<Record<string, unknown>>(`SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE status='open') as open, COUNT(*) FILTER (WHERE status='in_progress') as "inProgress" FROM legal_cases WHERE "companyId"=$1 AND "deletedAt" IS NULL`, [cid]);
    const [expiring] = await rawQuery<Record<string, unknown>>(`SELECT COUNT(*) as count FROM legal_contracts WHERE "companyId"=$1 AND "endDate" BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '30 days' AND status='active' AND "deletedAt" IS NULL`, [cid]);
    const [sessions] = await rawQuery<Record<string, unknown>>(`SELECT COUNT(*) as upcoming FROM legal_sessions ls JOIN legal_cases lc ON lc.id=ls."caseId" WHERE lc."companyId"=$1 AND lc."deletedAt" IS NULL AND ls."deletedAt" IS NULL AND ls."sessionDate" BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '7 days'`, [cid]);
    const [contingent] = await rawQuery<Record<string, unknown>>(`SELECT COALESCE(SUM("financialRisk"),0) as total FROM legal_cases WHERE "companyId"=$1 AND status NOT IN ('closed') AND "deletedAt" IS NULL`, [cid]).catch((e) => { logger.error(e, "legal query failed"); return [{ total: 0 }]; });
    res.json({

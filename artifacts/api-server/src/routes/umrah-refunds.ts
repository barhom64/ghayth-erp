// ─────────────────────────────────────────────────────────────────────────────
// umrah-refunds.ts — UMRAH REFUND REQUESTS (U-07 Phase 14)
//
// Routes carved verbatim from umrah-entities.ts into this dedicated sub-router.
// Mounted via `router.use(refundsRouter)` in umrah-entities.ts so the API
// surface stays identical (paths still resolve at /umrah/refund-requests...).
//
...
//
// Ledger-free: every write here targets the umrah-owned umrah_refund_requests
// table only (request lifecycle: requested → approved/rejected → paid →
// closed). The /pay route records settledAmount/treasuryId/paymentReference on
// the refund row but posts NO journal entry — the GL posting for the cash
// outflow lives in the finance/treasury module. treasuryId is a stored FK, not
// a cross-domain write. The transition state-machine lives in

...
saudiComplianceRouter.put(
  ...
  async (req, res) => {
    try {
      ...
      await sideEffect(scope, "wps.credentials.updated", "wps_bank_credential", 0, {
        companyId: scope.companyId,
        bankCode: format,
        channel: cfg.channel,
...
saudiComplianceRouter.delete(
  ...
  async (req, res) => {
    try {
      ...
      await sideEffect(scope, "wps.credentials.cleared", "wps_bank_credential", 0, {
        companyId: scope.companyId,
        bankCode: format,
      });
...
saudiComplianceRouter.post(
  ...
  async (req, res) => {
    try {
      ...
      await sideEffect(scope, "wps.run.created", "wps_run", wpsRunId, {
        period: input.period,
        bankCode: input.bankCode,
      });
...
saudiComplianceRouter.post(
  ...
  async (req, res) => {
    try {
      ...
      await sideEffect(scope, "wps.run.built", "wps_run", id, {
        ...outcome,
...
saudiComplianceRouter.post(
  ...
  async (req, res) => {
    try {
      ...
      await sideEffect(scope, "wps.run.submitted", "wps_run", id, {});
...
saudiComplianceRouter.post(
  ...
  async (req, res) => {
    try {
      ...
      await sideEffect(scope, "wps.run.sent_to_bank", "wps_run", id, {
...
saudiComplianceRouter.post(
  ...
  async (req, res) => {
    try {
      ...
      if (outcome.applied) {
        await sideEffect(scope, "wps.run.ack_polled", "wps_run", id, {
...
saudiComplianceRouter.post(
  ...
  async (req, res) => {
    try {
      ...
      await sideEffect(scope, "wps.run.acknowledged", "wps_run", id, { ...outcome });
      res.json({ data: outcome });
    } catch (err) {
...
saudiComplianceRouter.post(
  ...
  async (req, res) => {
    try {
      ...
      await sideEffect(scope, "mudad.settlement.retried", "mudad_settlement", id, {
...
async function sideEffect(
  scope: { companyId: number; userId: number; branchId: number | null },
  action: string,
  entity: string,
  entityId: number,
  ...
  createAuditLog({
    ...
    entityId,
    after,
  }).catch((e) => logger.error(e, "[saudi-compliance] audit failed"));
  emitEvent({
    companyId: scope.companyId,
    userId: scope.userId,
    action,

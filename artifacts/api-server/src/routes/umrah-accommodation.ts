// ─────────────────────────────────────────────────────────────────────────────
// umrah-accommodation.ts — UMRAH ACCOMMODATION (N6)
//
// U-07 Phase 4: the 9 hotel/room-block/allocation routes carved verbatim
// from umrah-entities.ts into a dedicated sub-router. Mounted via
// `router.use(accommodationRouter)` in umrah-entities.ts so the API surface
...
//   N6 from CRITICAL_DEFECTS_REPORT.md — replaces "hotelName free string
//   on umrah_pilgrims" with a real 3-table model:
//   umrah_hotels → umrah_room_blocks → umrah_room_allocations.
// ─────────────────────────────────────────────────────────────────────────────
...
router.post("/hotels", authorize({ feature: "umrah", action: "create" }), async (req, res) => {
  try {
    ...
    auditFromRequest(req, "create", "umrah_hotels", insertId, {
      after: { name: b.name, city: b.city },
    }).catch(() => undefined);
...
router.post("/room-blocks", authorize({ feature: "umrah", action: "create" }), async (req, res) => {
  try {
    ...
    auditFromRequest(req, "create", "umrah_room_blocks", insertId, {
      after: { hotelId: b.hotelId, seasonId: b.seasonId, totalRooms: b.totalRooms },
    }).catch(() => undefined);
...
router.post("/room-allocations", authorize({ feature: "umrah", action: "create" }), async (req, res) => {
  try {
    ...
    auditFromRequest(req, "create", "umrah_room_allocations", insertId, {
      after: { blockId: b.blockId, pilgrimId: b.pilgrimId },
    }).catch(() => undefined);
    res.status(201).json({ id: insertId, ok: true });

...
router.get("/", async (req, res) => {
  try {
    ...
    const [taskStats] = await rawQuery<Record<string, unknown>>(
      `SELECT
    ...
    const todayTasks = await rawQuery<Record<string, unknown>>(
    ...
    const pendingApprovals = await rawQuery<Record<string, unknown>>(
    ...
    let pendingFinanceApprovals: Record<string, unknown>[] = [];
    try {
      const { where: fw, params: fp } = buildFilter(scope, req);
      pendingFinanceApprovals = await rawQuery<Record<string, unknown>>(
    ...
    let pendingPurchaseRequests: Record<string, unknown>[] = [];
    try {
      const { where: pw, params: pp } = buildFilter(scope, req);
      pendingPurchaseRequests = await rawQuery<Record<string, unknown>>(
    ...
    const notifications = await rawQuery<Record<string, unknown>>(
...
router.get("/summary", async (req, res) => {
  try {
    ...
    try {
      const [o] = await rawQuery<Record<string, unknown>>(
    ...
    try {
      const [w] = await rawQuery<Record<string, unknown>>(

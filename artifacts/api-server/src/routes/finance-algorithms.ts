...
financeAlgorithmsRouter.get("/ar-aging", authorize({ feature: "finance.algorithms", action: "list" }), async (req, res) => {
  try {
    ...
    for (const inv of invoices) {
      ...
      const cid: number = (inv.clientId as number) ?? 0;
      const clientName = (inv.clientName as string | undefined) || `عميل #${cid}`;
...
financeAlgorithmsRouter.post("/bank-reconciliation/auto-match", authorize({ feature: "finance.algorithms", action: "create" }), async (req, res) => {
  try {
    ...
    for (const bRow of bankRows) {
      ...
      const date = new Date(bRow.statementDate as string);
...
financeAlgorithmsRouter.get("/fixed-assets/:id/schedule", authorize({ feature: "finance.algorithms", action: "list" }), async (req, res) => {
  try {
    ...
    const purchaseDate = new Date(asset.purchaseDate as string);
...
financeAlgorithmsRouter.post("/fixed-assets/:id/depreciate", authorize({ feature: "finance.algorithms", action: "create" }), async (req, res) => {
  try {
    ...
    const { journalId } = await financialEngine.postJournalEntry({
      ...
      lines: [
        { accountCode: (asset.depreciationAccountCode as string | undefined) ?? "6100", debit: depAmount, credit: 0, description: `إهلاك ${asset.name}` },
        { accountCode: (asset.accDepreciationAccountCode as string | undefined) ?? "1590", debit: 0, credit: depAmount, description: `مجمع إهلاك ${asset.name}` },
      ],
    });
  ...
  try {
    ...
    for (const asset of assets) {
      ...
      const { journalId } = await financialEngine.postJournalEntry({
        ...
        sourceKey: `finance:depreciation:${asset.id}:${targetPeriod}`,
        lines: [
          { accountCode: (asset.depreciationAccountCode as string | undefined) ?? "6100", debit: depAmount, credit: 0 },
          { accountCode: (asset.accDepreciationAccountCode as string | undefined) ?? "1590", debit: 0, credit: depAmount },
        ],
      });
...
financeAlgorithmsRouter.get("/inventory-costing/:productId", authorize({ feature: "finance.algorithms", action: "list" }), async (req, res) => {
  try {
    ...
    let runningQty = 0;
    let runningValue = 0;
    let waHistory: Record<string, unknown>[] = [];

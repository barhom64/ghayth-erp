...
/** Cancel a trip — frees vehicle+driver, no cost posted */
router.post("/trips/:id/cancel", requirePermission("fleet:update"), async (req, res) => {
  try {
    ...
    if (!trip) { res.status(404).json({ error: "الرحلة غير موجودة" }); return; }
    if (trip.status === "completed") {
      validationError(res, "الرحلة مكتملة بالفعل", "status", "لا يمكن إلغاء رحلة مكتملة");
      return;
    }
    if (trip.status === "cancelled") {
      validationError(res, "الرحلة ملغاة بالفعل", "status", "لا حاجة لإلغاء رحلة ملغاة");
      return;
    }
    if (!b.reason) {
      validationError(res, "سبب الإلغاء مطلوب", "reason", "أدخل سبب إلغاء الرحلة");
      return;
    }

    await rawExecute(
      `UPDATE fleet_trips SET status='cancelled', "endTime"=NOW(), notes=COALESCE(notes,'') || ' | إلغاء: ' || $1 WHERE id=$2`,
      [b.reason, tripId]
    );
    if (trip.vehicleId) {
      await rawExecute(`UPDATE fleet_vehicles SET status='available', "updatedAt"=NOW() WHERE id=$1 AND "companyId"=$2`, [trip.vehicleId, scope.companyId]);
    }
    if (trip.driverId) {
      await rawExecute(`UPDATE fleet_drivers SET status='available' WHERE id=$1 AND "companyId"=$2`, [trip.driverId, scope.companyId]);
    }
    ...
    await emitEvent({
      ...
      entity: "fleet_trips",
      entityId: tripId,
      details: `إلغاء رحلة #${tripId}: ${b.reason}`,
    });
    createAuditLog({
      companyId: scope.companyId, branchId: scope.branchId, userId: scope.userId,
      action: "cancel", entity: "fleet_trips", entityId: tripId,
      before: { status: trip.status }, after: { status: "cancelled", reason: b.reason },
    }).catch(console.error);

    const [updated] = await rawQuery<any>(`SELECT * FROM fleet_trips WHERE id=$1`, [tripId]);
    res.json({ ...updated, event: "fleet.trip.cancelled" });
  } catch (err) { handleRouteError(err, res, "Cancel trip error:"); }
});

  ...
  try {
    ...
    if (b.vehicleId) {
      ...
      if (vehicle) {
        ...
        const insuranceEnd = vehicle.insuranceEnd ? new Date(vehicle.insuranceEnd as string) : null;
    ...
    if (!selectedVehicleId) {
      ...
      if (vehicles.length > 0) {
        ...
        for (const v of vehicles) {
          let score = 0;
          const insuranceEnd = v.insuranceEnd ? new Date(v.insuranceEnd as string) : null;
    ...
    if (!selectedDriverId) {
      ...
      if (drivers.length > 0) {
        ...
        for (const d of drivers) {
          ...
          const hasValidLicense = d.licenseExpiry ? new Date(d.licenseExpiry as string) > new Date() : true;
    ...
    if (selectedVehicleId && !b.vehicleId) {
      ...
      if (autoVehicle) {
        const insuranceEnd = autoVehicle.insuranceEnd ? new Date(autoVehicle.insuranceEnd as string) : null;
        if (!insuranceEnd || insuranceEnd < new Date()) {
          throw new ValidationError(
            "لا يمكن بدء رحلة بمركبة تأمينها منتهي",
            {
  ...
  try {
    ...
    try {
      ...
      if (m.nextServiceDate) {
        ...
        const nextDate = new Date(m.nextServiceDate as string);
        await registerObligation({
          companyId: scope.companyId,
          branchId: scope.branchId ?? null,
...
router.get("/alerts", authorize({ feature: "fleet.vehicles", action: "list" }), async (req, res) => {
  try {
    ...
    const allInsurance = await rawQuery<Record<string, unknown>>(
    ...
    const expiringLicenses = await rawQuery<Record<string, unknown>>(
    ...
    const speedAlerts = await rawQuery<Record<string, unknown>>(
    ...
    const abnormalFuel = await rawQuery<Record<string, unknown>>(
    ...
    const frequentBreakdowns = await rawQuery<Record<string, unknown>>(
    ...
    const lowRatingDrivers = await rawQuery<Record<string, unknown>>(
    ...
    const oilDue = await rawQuery<Record<string, unknown>>(
...
router.patch("/insurance/:id", authorize({ feature: "fleet.vehicles", action: "update" }), async (req, res) => {
  try {
    ...
    if (endDate !== undefined) {
      ...
      if (existing.startDate && ed <= new Date(existing.startDate as string)) {
...
router.get("/stats", authorize({ feature: "fleet.vehicles", action: "list" }), async (req, res) => {
  try {
    const scope = req.scope!;
    const cid = scope.companyId;
    const [vehicles] = await rawQuery<Record<string, unknown>>(`SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE status='available') as available, COUNT(*) FILTER (WHERE status='in_use') as "inUse", COUNT(*) FILTER (WHERE status='maintenance') as "inMaintenance" FROM fleet_vehicles WHERE "companyId"=$1 AND "deletedAt" IS NULL`, [cid]);
    const [trips] = await rawQuery<Record<string, unknown>>(`SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE status='completed') as completed FROM fleet_trips WHERE "companyId"=$1 AND "deletedAt" IS NULL`, [cid]);
    const [fuel] = await rawQuery<Record<string, unknown>>(`SELECT COALESCE(SUM("totalCost"),0) as "totalFuelCost" FROM fleet_fuel_logs WHERE "companyId"=$1 AND "deletedAt" IS NULL`, [cid]);
    const [insurance] = await rawQuery<Record<string, unknown>>(`SELECT COUNT(*) as total FROM fleet_insurance WHERE "companyId"=$1 AND "deletedAt" IS NULL`, [cid]);
    const [maintenance] = await rawQuery<Record<string, unknown>>(`SELECT COUNT(*) as total FROM fleet_maintenance WHERE "companyId"=$1 AND "deletedAt" IS NULL`, [cid]);
    const [drivers] = await rawQuery<Record<string, unknown>>(`SELECT COUNT(*) as total FROM fleet_drivers WHERE "companyId"=$1 AND "deletedAt" IS NULL`, [cid]);
    const [alerts] = await rawQuery<Record<string, unknown>>(`SELECT COUNT(*) as total FROM fleet_maintenance WHERE "companyId"=$1 AND status='in_progress' AND "deletedAt" IS NULL`, [cid]);
    res.json({
  ...
  try {
    ...
    if ((b.lastServiceDate !== undefined || b.lastServiceMileage !== undefined) && b.nextServiceDate === undefined) {
      if (effectiveLastDate && existing.intervalDays) {
        const d = new Date(effectiveLastDate as string | number);
...
router.get("/vehicles/:id/tco", authorize({ feature: "fleet.vehicles", action: "list" }), async (req, res) => {
  try {
    ...
    const yearsSincePurchase = vehicle.purchaseDate
      ? (Date.now() - new Date(vehicle.purchaseDate as string).getTime()) / (365.25 * 24 * 3600 * 1000)
      : 1;

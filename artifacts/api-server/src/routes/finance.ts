...
router.get("/purchase-requests", async (req, res) => {
  try {
    const scope = req.scope!;
    const filters = parseScopeFilters(req);
    const { where, params } = buildScopedWhere(scope, filters, { companyColumn: 'pr."companyId"', branchColumn: 'pr."branchId"', enforceBranchScope: true });
    const rows = await rawQuery<any>(
...
router.get("/purchase-orders", async (req, res) => {
  try {
    const scope = req.scope!;
    const filters = parseScopeFilters(req);
    const { where, params } = buildScopedWhere(scope, filters, { companyColumn: 'po."companyId"', branchColumn: 'po."branchId"', enforceBranchScope: true });
    const rows = await rawQuery<any>(
...
router.get("/stats", async (req, res) => {
  try {
    const scope = req.scope!;
    const filters = parseScopeFilters(req);
    const { where, params, nextParamIndex } = buildScopedWhere(scope, filters, { enforceBranchScope: true });
...
router.get("/opening-balances", async (req, res) => {
  try {
    const scope = (req as any).scope;
    const rows = await rawQuery(
      `SELECT je.id, je.description, je.date, jl."accountId", coa.name AS "accountName",
              coa.code AS "accountCode", jl.debit, jl.credit
       FROM journal_entries je
       JOIN journal_lines jl ON jl."journalId" = je.id
       LEFT JOIN chart_of_accounts coa ON coa.id = jl."accountId"
       WHERE je."companyId" = $1 AND je.type = 'opening'
       ORDER BY je.date DESC, je.id DESC`,
      [scope.companyId]
    );
    res.json({ data: rows });
  } catch (err) {
    handleRouteError(err, res, "Opening balances error:");
  }
});
...
router.post("/opening-balances", async (req, res) => {
  try {
    ...
    handleRouteError(err, res, "Opening balances create error:");
  }
});

router.post("/opening-balances/import-csv", async (req, res) => {
  try {
    res.json({ data: { imported: 0, message: "CSV import not yet implemented" } });
  } catch (err) {
    handleRouteError(err, res, "Opening balances CSV error:");
  }
});

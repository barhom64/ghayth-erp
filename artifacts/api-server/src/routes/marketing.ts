...
router.post("/campaigns/:id/send", authorize({ feature: "marketing", action: "update" }), async (req, res) => {
  try {
    ...
    for (let i = 0; i < recipients.length; i += CHUNK) {
      ...
      // كل دفعة في معاملة واحدة (ذرّية): السجل + الطابور + صفوف المستلمين تُثبَّت
      // معاً أو تُلغى معاً؛ وبوابة عدم التكرار تجعل إعادة التشغيل بعد فشل جزئي آمنة.
      queued += await withTransaction(async () => {
      ...
      if (prepared.length === 0) return 0; // كل مستلمي هذه الدفعة سبق إرسالهم.

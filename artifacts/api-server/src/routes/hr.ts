...
router.patch("/leave-requests/:id/approve", requirePermission("hr:update"), async (req, res) => {
  ...
  try {
    ...
    if (approved === "returned") {
      if (!reason) { throw new ValidationError("يجب ذكر سبب الإرجاع", { field: "reason" }); }
...
router.patch("/leave-requests/:id/escalate", requirePermission("hr:update"), async (req, res) => {
  try {
    ...
    if (!request) { throw new NotFoundError("الطلب غير موجود أو ليس معلقاً"); }
    ...
    if (expiresAt > new Date()) {
      ...
      throw new ConflictError(
        ...
        { meta: { expiresAt: expiresAt.toISOString(), hoursRemaining } }
      );
    }
...
router.post("/approval-chain-definitions", requirePermission("hr:create"), async (req, res) => {
  try {
    ...
    if (!name || !chainType) {
      throw new ValidationError("الاسم ونوع السلسلة مطلوبان");
    }
    const validTypes = ["leaves", "purchases", "expenses", "advances", "letters"];
    if (!validTypes.includes(chainType)) {
      throw new ValidationError(`نوع السلسلة يجب أن يكون أحد: ${validTypes.join(", ")}`, { field: "type" });
    }
...
router.patch("/approval-requests/:id/decide", requirePermission("hr:update"), async (req, res) => {
  try {
    ...
    if (!request) { throw new NotFoundError("طلب الموافقة غير موجود أو تمت معالجته"); }
...
router.patch("/leave-requests/:id", requirePermission("hr:update"), async (req, res) => {
  try {
    ...
    if (status && ["approved", "rejected"].includes(status)) {
      throw new ValidationError("استخدم نقطة نهاية الموافقة/الرفض المخصصة");
    }
    const sets: string[] = [];
    const params: any[] = [];
    let idx = 1;
    ...
    if (!row) { throw new NotFoundError("طلب الإجازة غير موجود"); }
    res.json(row);
  } catch (err) { handleRouteError(err, res, "خطأ غير متوقع"); }
});
...
router.patch("/official-letters/:id/approve", requirePermission("hr:update"), async (req, res) => {
  try {
    ...
    if (newStatus === "rejected" && !notes) {
      throw new ValidationError("يجب ذكر سبب الرفض", { field: "reason" });
    }
...
router.put("/onboarding-steps", requirePermission("hr:update"), async (req, res) => {
  try {
    ...
    if (!Array.isArray(steps)) { throw new ValidationError("الخطوات مطلوبة", { field: "steps" }); }
...
router.post("/impact-preview/termination", requirePermission("hr:read"), async (req, res) => {
  try {
    ...
    if (!employeeId) { throw new ValidationError("معرف الموظف مطلوب", { field: "employeeId" }); }
    const [assignment] = await rawQuery<any>(
      `SELECT id FROM employee_assignments WHERE "employeeId" = $1 AND "companyId" = $2 AND status = 'active' LIMIT 1`,
      [Number(employeeId), scope.companyId]
    );
...
router.post("/evaluation-cycles", requirePermission("hr:create"), async (req, res): Promise<any> => {
  try {
    const scope = req.scope!;
    if (!isHR(scope)) throw new ForbiddenError("مسموح فقط لـ HR بإنشاء دورات التقييم");

    const { employeeId, period, notes, participants = [] } = req.body as any;
    if (!employeeId || !period) throw new ValidationError("employeeId و period مطلوبان");
    ...
    if (!subjectAssign) throw new ValidationError("الموظف لا ينتمي إلى هذه الشركة");
...
router.post("/evaluation-cycles/:id/upward-review", requirePermission("hr:create"), async (req, res): Promise<any> => {
  try {
    ...
    if (!managerId || !overallScore) throw new ValidationError("managerId و overallScore مطلوبان");
...
router.post("/accruals/monthly", requirePermission("hr:update"), async (req, res) => {
  try {
    ...
    const periodCheck = await checkFinancialPeriodOpen(scope.companyId, accrualDate);
    if (!periodCheck.open) {
      throw new ValidationError(`لا يمكن تسجيل استحقاقات في فترة مُقفلة: ${periodCheck.periodName ?? ""}`);
    }
    ...
    if (existing) {
      throw new ConflictError("تم تسجيل استحقاقات هذه الفترة مسبقاً", { meta: { journalId: existing.id } });
    }

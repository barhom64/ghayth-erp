...
export async function getJourneyProgress(
  ...
  return { id: row.id as number, completedSteps: steps, totalSteps: row.totalSteps as number, status: row.status as string, progress: steps.length / (row.totalSteps as number) };
}

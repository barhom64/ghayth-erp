...
export async function parseMutamersWorkbook(buffer: Buffer): Promise<ParsedRow[]> {
  return parseWorkbook(buffer, MUTAMER_HEADER_MAP, "mutamers");
}

export async function parseVouchersWorkbook(buffer: Buffer): Promise<ParsedRow[]> {
...
async function parseWorkbook(buffer: Buffer, headerMap: Record<string, string>, fileType: string): Promise<ParsedRow[]> {
  const raw = (await parseFirstSheetAOA(buffer)) as any[][];
  if (raw.length === 0) throw new ValidationError("الملف لا يحتوي على أي ورقة");

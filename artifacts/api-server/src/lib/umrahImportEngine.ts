...
export async function confirmVouchersImport(
  ...
  return withTransaction(async (client) => {
    ...
    // info. mutamerCount is NOT synced here (vouchers don't create
    // pilgrims, so the count is owned by the mutamers path). Savepoint-
    // guarded so a backfill hiccup can't roll back committed import work.
    try {
      await client.query("SAVEPOINT sp_group_backfill");
      await backfillGroupLinks(client, scope, Array.from(touchedGroupIds));
      await client.query("RELEASE SAVEPOINT sp_group_backfill");
    } catch (e) {
      await client.query("ROLLBACK TO SAVEPOINT sp_group_backfill");
      await client.query("RELEASE SAVEPOINT sp_group_backfill");
...
async function backfillGroupLinks(
  ...
  if (opts.syncCounts) {
    // Keep mutamerCount honest with the pilgrims actually linked so the
    // groups list "معتمرون" column, the delete-guard, and the merge dialog
    // all read the real number after an import.
    await client.query(
      `UPDATE umrah_groups g

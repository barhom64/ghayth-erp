...
export const EVENT_CATALOG: EventDefinition[] = [
  ...
  { name: "fiscal_period.locked", label: "قفل دائم لفترة مالية", domain: "finance", description: "تُصدر عند قفل فترة مالية بشكل دائم بعد التدقيق (PER-3) — لا يمكن إعادة فتحها بعد القفل", payload: {"id":"number"}, consumers: ["auditTrail"], sideEffects: ["audit"], critical: true },
  ...
  { name: "finance.insurance.premium_opened", label: "فتح قسط تأمين", domain: "finance", description: "تُصدر عند ترحيل قسط تأمين (عقاري/طبي) كرصيد مدفوع مقدماً وفتح جدول إطفائه (#2249)", payload: {"journalId":"number","scheduleId":"number","premiumAmount":"number"}, consumers: ["auditTrail"], sideEffects: ["gl_post","audit"] },
  ...
  { name: "umrah.season.closed", label: "إغلاق موسم عمرة", domain: "umrah", description: "تُصدر عند إغلاق الموسم — PATCH /seasons/:id status=closed", payload: {"status":"string"}, consumers: ["auditTrail"], sideEffects: ["audit"] },
  { name: "umrah.season.archived", label: "أرشفة موسم عمرة", domain: "umrah", description: "تُصدر عند أرشفة الموسم — PATCH /seasons/:id status=archived", payload: {"status":"string"}, consumers: ["auditTrail"], sideEffects: ["audit"] },
  { name: "umrah.overstay.detected", label: "رصد تجاوز إقامة", domain: "umrah", description: "تُصدر عند رصد معتمر متجاوز للمدة", payload: {"id":"number"}, consumers: ["auditTrail"], sideEffects: ["audit","notification"] },
  { name: "umrah.absconder.detected", label: "رصد تغيّب معتمر", domain: "umrah", description: "تُصدر عند رصد معتمر متغيّب", payload: {"id":"number"}, consumers: ["auditTrail"], sideEffects: ["audit","notification"] },
  ...
  { name: "umrah.violation.updated", label: "تحديث مخالفة", domain: "umrah", description: "تُصدر عند تحديث مخالفة", payload: {"id":"number"}, consumers: ["auditTrail"], sideEffects: ["audit"] },
  ...
  // ─── FLEET TELEMATICS (#1354 — CMSV6 / AI MDVR / Sensors) ─────────────────
  { name: "fleet.telematics.position.updated", label: "تحديث موقع GPS", domain: "fleet", description: "تُصدر عند استلام نقطة GPS جديدة من جهاز MDVR", payload: { deviceId: "number", vehicleId: "number", lat: "number", lng: "number", speed: "number" }, consumers: ["fleetLiveMap", "alertRules"], sideEffects: ["audit"] },
  { name: "fleet.telematics.device.online", label: "اتصال جهاز MDVR", domain: "fleet", description: "تُصدر عند رصد جهاز MDVR على الشبكة بعد انقطاع", payload: { deviceId: "number", vehicleId: "number" }, consumers: ["fleetLiveMap"], sideEffects: ["audit", "notification"] },
  { name: "fleet.telematics.device.offline", label: "انقطاع جهاز MDVR", domain: "fleet", description: "تُصدر عند فقدان الاتصال بجهاز MDVR", payload: { deviceId: "number", vehicleId: "number", lastSeenAt: "string" }, consumers: ["fleetLiveMap", "alertRules"], sideEffects: ["audit", "notification"] },
  { name: "fleet.telematics.ai.dms_alert", label: "تنبيه DMS", domain: "fleet", description: "تنبيه مراقبة السائق (نعاس/تشتت/هاتف/تدخين)", payload: { alertId: "number", deviceId: "number", vehicleId: "number", driverId: "number", alertType: "string", severity: "string" }, consumers: ["fleetAlerts", "disciplineEngine"], sideEffects: ["audit", "notification"], critical: true },
  { name: "fleet.telematics.ai.adas_alert", label: "تنبيه ADAS", domain: "fleet", description: "تنبيه مساعدة القيادة (اصطدام أمامي/خروج عن المسار/مسافة قصيرة)", payload: { alertId: "number", deviceId: "number", vehicleId: "number", alertType: "string", severity: "string" }, consumers: ["fleetAlerts", "disciplineEngine"], sideEffects: ["audit", "notification"], critical: true },
  { name: "fleet.telematics.ai.bsd_alert", label: "تنبيه BSD", domain: "fleet", description: "تنبيه النقطة العمياء عند المناورة", payload: { alertId: "number", deviceId: "number", vehicleId: "number", severity: "string" }, consumers: ["fleetAlerts"], sideEffects: ["audit", "notification"] },
  { name: "fleet.telematics.sensor.fuel_changed", label: "تغيّر مستوى الوقود", domain: "fleet", description: "تُصدر عند تغيّر ملحوظ في حساس الوقود (ملء/تفريغ مفاجئ)", payload: { readingId: "number", vehicleId: "number", readingValue: "number" }, consumers: ["fuelTracker", "alertRules"], sideEffects: ["audit"] },
  { name: "fleet.telematics.sensor.weight_changed", label: "تغيّر الوزن", domain: "fleet", description: "تُصدر عند تغيّر قراءة حساس الوزن في القلاب", payload: { readingId: "number", vehicleId: "number", readingValue: "number" }, consumers: ["alertRules"], sideEffects: ["audit"] },
  { name: "fleet.telematics.sensor.pto_on", label: "تشغيل PTO", domain: "fleet", description: "تُصدر عند تفعيل PTO (Power Take-Off)", payload: { readingId: "number", vehicleId: "number" }, consumers: ["operationsCenter"], sideEffects: ["audit"] },
  { name: "fleet.telematics.sensor.pto_off", label: "إيقاف PTO", domain: "fleet", description: "تُصدر عند فصل PTO", payload: { readingId: "number", vehicleId: "number" }, consumers: ["operationsCenter"], sideEffects: ["audit"] },
  { name: "fleet.telematics.sensor.dump_piston_up", label: "رفع بستم القلاب", domain: "fleet", description: "تُصدر عند رفع صندوق القلاب (بستم)", payload: { readingId: "number", vehicleId: "number" }, consumers: ["operationsCenter"], sideEffects: ["audit"] },
  { name: "fleet.telematics.sensor.dump_piston_down", label: "خفض بستم القلاب", domain: "fleet", description: "تُصدر عند إنزال صندوق القلاب", payload: { readingId: "number", vehicleId: "number" }, consumers: ["operationsCenter"], sideEffects: ["audit"] },
  { name: "fleet.telematics.video.session_started", label: "بدء بث مباشر", domain: "fleet", description: "تُصدر عند فتح جلسة بث فيديو من كاميرا المركبة", payload: { sessionId: "number", deviceId: "number", vehicleId: "number", channelNo: "number", requestedBy: "number" }, consumers: ["auditTrail"], sideEffects: ["audit"] },
  { name: "fleet.telematics.video.session_stopped", label: "إيقاف بث مباشر", domain: "fleet", description: "تُصدر عند إغلاق جلسة بث فيديو", payload: { sessionId: "number", deviceId: "number" }, consumers: ["auditTrail"], sideEffects: ["audit"] },

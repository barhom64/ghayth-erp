...
-- Ensure required columns exist on pre-existing tables before indexing
ALTER TABLE fx_rates ADD COLUMN IF NOT EXISTS "effectiveDate" DATE;
ALTER TABLE fx_rates ADD COLUMN IF NOT EXISTS source VARCHAR(40) DEFAULT 'manual';
UPDATE fx_rates SET "effectiveDate" = COALESCE("effectiveDate", "rateDate"::date, CURRENT_DATE) WHERE "effectiveDate" IS NULL;

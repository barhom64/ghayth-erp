...
/**
 * U-07 Phase 12 — umrah-entities.ts split smoke.
 *
 * Scope:
 *   - Carves the 2 letter routes (PDF + dispatch) into a dedicated sub-router:
 *       artifacts/api-server/src/routes/umrah-letters.ts
 *   - The parent `umrah-entities.ts` mounts the sub-router via
 *     `router.use(lettersRouter)` so the API surface stays identical
 *     (all paths still resolve under /umrah/letters/...).
 *
 * Non-goals (Permanent Hard Rails):
 *   - No engine touch.
 *   - No route body change — pure code move + mount.
 *   - No API surface change.
 *   - No ledger touch — the dispatch UPDATE only flips letter status/sentAt.
 *
...
const CHILD = readFileSync(
  join(REPO_ROOT, "artifacts/api-server/src/routes/umrah-letters.ts"),
  "utf8",
);
...
// ─────────────────────────────────────────────────────────────────────────────
describe("U-07 Phase 12 §A — umrah-letters.ts is a valid sub-router file", () => {
...
// ─────────────────────────────────────────────────────────────────────────────
describe("U-07 Phase 12 §B — parent mounts the sub-router", () => {
  it("parent imports the new module as a default import", () => {
    expect(PARENT).toMatch(
      /import\s+lettersRouter\s+from\s+["']\.\/umrah-letters\.js["']/,
    );
  });

  it("parent mounts the sub-router with router.use(lettersRouter)", () => {
    expect(PARENT).toMatch(/router\.use\(\s*lettersRouter\s*\)/);
  });
});
...
// ─────────────────────────────────────────────────────────────────────────────
describe("U-07 Phase 12 §C — both moved routes are present in the child", () => {
...
// ─────────────────────────────────────────────────────────────────────────────
describe("U-07 Phase 12 §D — parent no longer declares the moved routes", () => {
  const routes: [string, string][] = [
    ["get", "/letters/:id/pdf"],
    ["post", "/letters/:id/dispatch"],
  ];
...
// ─────────────────────────────────────────────────────────────────────────────
describe("U-07 Phase 12 §E — parent shrunk below the pre-split line count", () => {
  it("parent file has fewer than 3910 lines (started ~3974 before this carve, ~3872 after)", () => {
    const lineCount = PARENT.split("\n").length;
    expect(lineCount).toBeLessThan(3910);
  });
});

...
describe("githubSupportSync — جسم الـIssue", () => {
  it("يحوي المرجع + الفئة + مقدّم الشكوى + الوصف + رابطًا عميقًا + استدعاء @claude", () => {
    const body = buildIssueBody(TECH_TICKET, "أحمد · 0500000000", "https://erp.door.sa", true);
    ...
    expect(body).toContain("https://erp.door.sa/support/42");
    expect(body).toContain("@claude"); // يُفعّل وكيل ghayth-claude-agent
  });

  it("بلا @claude عند تعطيل المساعد (mentionAssistant=false)", () => {
    expect(buildIssueBody(TECH_TICKET, "—", "https://erp.door.sa", false)).not.toContain("@claude");
  });
});

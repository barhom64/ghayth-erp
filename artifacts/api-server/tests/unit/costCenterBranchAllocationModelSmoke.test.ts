...
describe("الدفعة 2 — محرّك الرواتب يستمّ مركز التكلفة (بُعد فقط)", () => {
  ...
  it("فرع breakdown هو المصدر الموثوق + التجاوز الصريح يفوز ثم الاشتقاق", () => {
    expect(HR_ENGINE).toMatch(/for \(const e of payroll\.breakdown\) empBranch\.set\(e\.employeeId, e\.branchId \?\? null\)/);
    expect(HR_ENGINE).toMatch(/empOverride\.get\(empId\) \?\? \(await ccForBranch\(branchId\)\)/);
  });
  it("التجاوز مُقيّد بمطابقة فرع الرواتب (يحمي من صفوف منح الوصول)", () => {
    expect(HR_ENGINE).toMatch(/r\.branchId === empBranch\.get\(r\.employeeId\)/);
  });

...
describe("الدفعة 2 — محرّك الرواتب يستمّ مركز التكلفة (بُعد فقط)", () => {
  ...
  it("الاستمام مشروط بـ breakdown موثوق (لا يمسّ المبالغ)", () => {
    expect(HR_ENGINE).toMatch(/if \(payroll\.breakdown && payroll\.breakdown\.length > 0 && breakdownTrusted\)[\s\S]{0,600}employee_branch_allocations/);
  });
  it("التجاوز الصريح يفوز ثم المركز المشتق من الفرع", () => {
    expect(HR_ENGINE).toMatch(/let cc = r\.costCenterId \?\? null;[\s\S]{0,300}deriveBranchCostCenter\(ctx\.companyId, r\.branchId\)/);
  });
  it("الاستمام يضبط costCenterId/branchId فقط على سطور الموظف", () => {
    expect(HR_ENGINE).toMatch(/const stamp = \([\s\S]{0,400}l\.costCenterId = d\.costCenterId/);
    expect(HR_ENGINE).toMatch(/for \(const l of debitLines\) stamp\(l\);[\s\S]{0,80}for \(const l of deductionLines\) stamp\(l\);/);
  });
});

...
/**
 * U-07 Phase 4 — umrah-entities.ts split, accommodation CRUD carve-out.
 *
 * Mirrors Phase 2 (families) and Phase 3 (refunds) smoke. Pins:
 *   - The new sub-router file exists + is a valid Router export.
 *   - The parent mounts the sub-router (so /umrah/hotels, /room-blocks/…
 *     still resolves).
 *   - All 9 accommodation routes live in the child.
...
const ACCOMMODATION_ROUTES: Array<[string, string]> = [
  ["get", "/hotels"],
  ["post", "/hotels"],
  ["patch", "/hotels/:id"],
  ["delete", "/hotels/:id"],
...
// ─────────────────────────────────────────────────────────────────────────────
describe("U-07 Phase 4 §A — umrah-accommodation.ts is a valid sub-router file", () => {
...
describe("U-07 Phase 4 §B — parent mounts the sub-router", () => {
  it("parent imports the new module as a default import", () => {
    expect(PARENT).toMatch(
      /import\s+accommodationRouter\s+from\s+["']\.\/umrah-accommodation\.js["']/,
    );
  });

  it("parent mounts the sub-router with router.use(accommodationRouter)", () => {
    expect(PARENT).toMatch(/router\.use\(\s*accommodationRouter\s*\)/);
  });
});
...
describe("U-07 Phase 4 §C — all 9 accommodation routes are present in the child", () => {
  for (const [verb, route] of ACCOMMODATION_ROUTES) {
...
// ─────────────────────────────────────────────────────────────────────────────
describe("U-07 Phase 4 §E — parent shrank from the pre-carve line count", () => {
  it("parent file has fewer than 5900 lines (pre-Phase-4 it was 6026+)", () => {
    const lineCount = PARENT.split("\n").length;
    expect(lineCount).toBeLessThan(5900);
  });
});

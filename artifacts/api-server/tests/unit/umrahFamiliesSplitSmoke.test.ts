...
// ─────────────────────────────────────────────────────────────────────────────
describe("U-07 Phase 2 §E — parent shrank from the pre-carve line count", () => {
  it("parent file has fewer than 6100 lines (pre-Phase-2 it was 6218)", () => {
    const lineCount = PARENT.split("\n").length;
    expect(lineCount).toBeLessThan(6100);
  });
});

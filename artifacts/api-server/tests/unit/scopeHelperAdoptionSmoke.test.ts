...
const MANUAL_SCOPE_ALLOWLIST = new Set<string>([
  ...
  "umrah-entities.ts",
  // umrah-hotels.ts: U-07 Phase 4 split — 9 accommodation routes (hotels CRUD
  // + room-blocks list/create + per-block allocations list + allocation
  // create/delete) carved verbatim out of umrah-entities.ts. Point lookups +
  // per-tenant CRUD keyed on (companyId, id); inherits the same allowlist
...
describe("scope helper adoption ratchet — GAP_MATRIX #13", () => {
  ...
  it("snapshot: current adoption rate", () => {
    ...
    expect({ total, helperUsers, manualOnly }).toEqual({
      ...
      // justification as the parent.
      // +1 total/manualOnly: U-07 Phase 4 routes/umrah-hotels.ts — 9
      // accommodation routes (hotels CRUD + room-blocks + allocations)
      // carved verbatim out of umrah-entities.ts. Same allowlist
      // justification as the parent.
      total: 128,
      helperUsers: 39,
      manualOnly: 86,

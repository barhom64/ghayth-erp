// FIN-SUB-03b (#2118) slice 2 — the EXPENSES LIST read surface
// (GET /api/finance/expenses) now returns the three status axes
// (documentStatus / paymentStatus / postingStatus) ALONGSIDE the legacy
// status + isPaid (both KEPT, nothing removed). Proves the new read is MORE
// CORRECT in the known-misleading case: a directly-posted expense carries
...
// over REAL HTTP on the live DB — only the read surface is exercised; no
// payment/posting/trigger behaviour is changed. The remaining journal reads
// stay on the legacy column and are documented as a follow-up list (see PR),
// to be converted under independent permission per slice. Test cluster only.
...
const BRANCH = 2;
const PFX = "EXP-test-fin03b2-"; // expenses list filters ref LIKE 'EXP%'
const CASH = "1111";
const SUBCASH = "1112";

d("FIN-SUB-03b slice 2 — expenses list surfaces the three status axes (live DB, HTTP)", () => {
  let request: any;
  let app: any;
  ...
  async function makeExpense(suffix: string) {
    const ref = PFX + suffix;
    const id = await createJournalEntry({
      companyId: COMPANY, branchId: BRANCH, createdBy: created.assignmentId,
      ref, description: "fin03b2 " + suffix, sourceType: "test", sourceKey: ref,
  ...
  it("a directly-posted expense (status='draft', balancesApplied=true) reads postingStatus='posted' in the list — where the legacy status alone misleads", async () => {
    const id = await makeExpense("direct");
    ...
    expect(mine, "the created expense must appear in the list read").toBeTruthy();

    // legacy columns KEPT (not removed)
    expect(mine.status).toBe("draft");
    expect(mine).toHaveProperty("isPaid");
    ...
    // the new read is MORE CORRECT: this expense IS posted
  ...
  it("the legacy status + isPaid stay present in the same read (additive, nothing removed)", async () => {
    const id = await makeExpense("kept");
    const res = await request(app)
      .get("/api/finance/expenses")
      .set("Authorization", `Bearer ${token}`);
    expect(res.status).toBe(200);

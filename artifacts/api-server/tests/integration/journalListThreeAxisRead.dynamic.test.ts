// FIN-SUB-03b (#2118) slice 1 — the journal-entries LIST read surface
// (GET /api/finance/journal) now returns the three status axes
// (documentStatus / paymentStatus / postingStatus) ALONGSIDE the legacy
// `status` (kept, never removed). This proves the new read is MORE CORRECT
// in the known-misleading case the owner flagged: a directly-posted entry
// carries status='draft' WITH balancesApplied=true, so the legacy `status`
// alone reads it as a draft/unposted entry, while `postingStatus` reads it
// truthfully as 'posted' (derived from balancesApplied by the migration-311
// trigger). Driven over REAL HTTP on the live DB — only the read surface is
// exercised; no payment/posting/trigger behaviour is changed. The remaining
// journal reads stay on the legacy column and are documented as a follow-up
// list (see the PR), to be converted under independent permission per slice.
...
const BRANCH = 2;
const PFX = "test-fin03b-";
const CASH = "1111";
const SUBCASH = "1112";

d("FIN-SUB-03b slice 1 — journal list read surfaces the three status axes (live DB, HTTP)", () => {
  let request: any;
  let app: any;
  ...
  async function cleanup() {
    ...
    // also catch any rows left from a prior aborted run, by ref prefix
  ...
  it("a directly-posted entry (status='draft', balancesApplied=true) reads postingStatus='posted' in the list — where the legacy status alone misleads", async () => {
    const ref = PFX + "direct";
    const id = await createJournalEntry({
      companyId: COMPANY, branchId: BRANCH, createdBy: created.userId,
      ref, description: "fin03b direct", sourceType: "test", sourceKey: ref,
      lines: [
    ...
    // Sanity at the DB level: this IS the misleading shape — legacy status
    // 'draft' but the balances ARE applied, so the axis says 'posted'.
    ...
    // The READ SURFACE under test: GET /api/finance/journal.
    ...
    expect(mine, "the created entry must appear in the list read").toBeTruthy();

    // Legacy column is KEPT (not removed) and still reads 'draft'.
    ...
    // And the new read is MORE CORRECT: this entry IS posted. The legacy
    // `status` says 'draft' (would mislabel it as unposted); postingStatus
    // tells the truth.
  ...
  it("the legacy `status` field is still present in the same read (additive, nothing removed)", async () => {
    const ref = PFX + "kept";
    const id = await createJournalEntry({
      companyId: COMPANY, branchId: BRANCH, createdBy: created.userId,
      ref, description: "fin03b kept", sourceType: "test", sourceKey: ref,
      lines: [
        { accountCode: CASH, debit: 5, credit: 0 },
        { accountCode: SUBCASH, debit: 0, credit: 5 },
      ],
    });
    journalIds.push(id);
    ...
    // both the legacy field AND all three axes coexist on the row

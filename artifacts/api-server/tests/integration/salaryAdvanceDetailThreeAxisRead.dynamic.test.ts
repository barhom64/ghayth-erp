// FIN-SUB-03b (#2118) slice 6 — the SALARY-ADVANCE DETAIL read surface
// (GET /api/finance/salary-advances/:id) now returns the three status axes
...
// payment/posting/trigger behaviour changes. The advances LIST was already
// done (#2160) and is NOT touched here; opening balances, the FE, and the
// remaining reads are NOT in this slice — documented as a follow-up list (see
// PR), each converted under independent permission. Test cluster only.
...
const BRANCH = 2;
const PFX = "SALARY-ADV-test-fin03b6-"; // advance detail filters ref LIKE 'SALARY-ADV%'
const CASH = "1111";
const SUBCASH = "1112";

d("FIN-SUB-03b slice 6 — salary-advance detail surfaces the three status axes (live DB, HTTP)", () => {
  let request: any;
  let app: any;
  ...
  async function makeAdvance(suffix: string) {
    ...
    const id = await createJournalEntry({
      ...
      ref, description: "fin03b6 " + suffix, sourceType: "test", sourceKey: ref,
  ...
  it("a directly-posted advance (status='draft', balancesApplied=true) reads postingStatus='posted' in the DETAIL — where the legacy status alone misleads", async () => {
    ...
    expect(a).toHaveProperty("documentStatus");
    expect(a).toHaveProperty("paymentStatus");
    expect(a).toHaveProperty("postingStatus");

    // the new read is MORE CORRECT: this advance IS posted
    expect(a.postingStatus).toBe("posted");
    expect(a.status).not.toBe(a.postingStatus); // the divergence is the whole point
    expect(a.documentStatus).toBe("draft");     // approval lifecycle from legacy status
    expect(a.paymentStatus).toBe("unpaid");     // canBePaid: a draft is never paid
  });

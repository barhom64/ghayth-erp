// FIN-SUB-03b (#2118) slice 3 — the VOUCHERS LIST read surface
// (GET /api/finance/vouchers) now returns the three status axes
...
// payment/posting/trigger behaviour changes. /vouchers/:id (detail) is NOT in
// this slice; remaining reads stay on the legacy column and are documented as
// a follow-up list (see PR), each converted under independent permission.
// Test cluster only.
...
const BRANCH = 2;
const PFX = "PV-test-fin03b3-"; // vouchers list filters ref LIKE 'RV%' OR 'PV%'
const CASH = "1111";
const SUBCASH = "1112";

d("FIN-SUB-03b slice 3 — vouchers list surfaces the three status axes (live DB, HTTP)", () => {
  let request: any;
  let app: any;
  ...
  async function makeVoucher(suffix: string) {
    const ref = PFX + suffix; // starts with PV → matches the vouchers filter
    const id = await createJournalEntry({
      companyId: COMPANY, branchId: BRANCH, createdBy: created.assignmentId,
      ref, description: "fin03b3 " + suffix, sourceType: "test", sourceKey: ref,
  ...
  it("a directly-posted voucher (status='draft', balancesApplied=true) reads postingStatus='posted' in the list — where the legacy status alone misleads", async () => {
    ...
    expect(mine, "the created voucher must appear in the list read").toBeTruthy();

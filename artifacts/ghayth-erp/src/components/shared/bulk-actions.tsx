...
export function BulkCheckbox({ checked, indeterminate, onChange, className }: { checked: boolean; indeterminate?: boolean; onChange: () => void; className?: string }) {
  ...
  return (
    <button onClick={onChange} className={cn("text-muted-foreground hover:text-status-info-foreground transition-colors p-0.5", checked && "text-status-info-foreground", className)}>
...
export function BulkActionsBar({
  ...
  return (
    <div className="flex items-center gap-3 p-3 bg-status-info-surface border border-status-info-surface rounded-lg animate-in fade-in slide-in-from-top-1">
      ...
      <div className="flex items-center gap-1.5 ms-auto">
        {actions.includes("approve") && (
          <Button size="sm" variant="outline" className="gap-1 text-status-success-foreground border-status-success-surface hover:bg-status-success-surface" onClick={() => executeBulk("approve")} disabled={!!loading}>
            {loading === "approve" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <CheckCircle className="h-3.5 w-3.5" />}
            اعتماد جماعي
          </Button>
        )}
        {actions.includes("reject") && (
          <Button size="sm" variant="outline" className="gap-1 text-status-error-foreground border-status-error-surface hover:bg-status-error-surface" onClick={() => executeBulk("reject")} disabled={!!loading}>
            {loading === "reject" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <XCircle className="h-3.5 w-3.5" />}
            رفض جماعي
          </Button>
        )}
        {actions.includes("close") && (
          <Button size="sm" variant="outline" className="gap-1 text-gray-700 border-border hover:bg-surface-subtle" onClick={() => executeBulk("close")} disabled={!!loading}>
        ...
        {actions.includes("export") && csvColumns && (
          <Button size="sm" variant="outline" className="gap-1 text-status-info-foreground border-status-info-surface hover:bg-status-info-surface" onClick={handleExport} rateLimitAware>
            <Download className="h-3.5 w-3.5" />
            تصدير جدولي
          </Button>
        )}
        {actions.includes("delete") && (
          <Button size="sm" variant="outline" className="gap-1 text-status-error-foreground border-status-error-surface hover:bg-status-error-surface" onClick={() => executeBulk("delete")} disabled={!!loading}>

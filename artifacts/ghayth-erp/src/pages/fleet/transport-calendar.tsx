/**
 * TR-022 — Unified Transport Calendar (SPA).
 *
 * Mirrors the umrah operational calendar (pages/umrah/calendar.tsx) for
 * the fleet/transport domain: a monthly + yearly grid that aggregates
 * the transport pipeline's date-bearing entities into toggleable layers,
 * plus a day-detail side panel for drill-down.
 *
 * Five layers (toggleable), served by GET /transport/calendar/events:
 *   • حجوزات النقل        transport_bookings.requestedPickupDate     — أزرق
 *   • أوامر التشغيل        transport_dispatch_orders.scheduledStartAt — بنفسجي
 *   • استحقاقات المركبات   fleet_vehicles expiries                    — أصفر
 *   • عقود التأجير         fleet_rental_contracts start/end           — أخضر
 *   • شحنات البضائع        cargo_manifests pickup/delivery            — رمادي
 */
...
// Drill-down URL per layer. Booking/rental/cargo open the row detail when
// a sample id is present; the rest open their list/board page.
const LAYER_HREF: Record<CalendarLayer, (ids: number[]) => string> = {
  booking:     (ids) => ids[0] ? `/fleet/transport/bookings/${ids[0]}` : `/fleet/transport/bookings`,
  dispatch:    ()    => `/fleet/transport/ops-dashboard`,
  maintenance: ()    => `/fleet/maintenance`,
  rental:      (ids) => ids[0] ? `/fleet/rental-contracts/${ids[0]}` : `/fleet/rental-contracts`,
  cargo:       (ids) => ids[0] ? `/fleet/cargo/${ids[0]}` : `/fleet/cargo`,
};
...
export default function TransportCalendar() {
  ...
  const [enabledLayers, setEnabledLayers] = useState<Set<CalendarLayer>>(
    new Set(["booking", "dispatch", "maintenance", "rental", "cargo"]),
  );
  ...
  const eventsQ = useApiQuery<CalendarResp>(
    ["transport-calendar", from, to, layersQs],
    `/transport/calendar/events?from=${from}&to=${to}&layers=${layersQs}`,
    {
  ...
  return (
    <PageShell
      title="تقويم النقل الموحّد"
      subtitle="نظرة يومية على الحجوزات، أوامر التشغيل، استحقاقات المركبات، التأجير، والشحنات"
      breadcrumbs={[{ href: "/fleet", label: "الأسطول" }, { label: "التقويم" }]}
    >
      <FleetTabsNav />
      ...
      <div className="grid grid-cols-1 md:grid-cols-[1fr_320px] gap-3">
        ...
        <Card>
          <CardContent className="p-4 space-y-3">
            ...
            {detailEvents.map((ev) => {
              const colorCls = COLOR_CLASSES[ev.color];
              const href = LAYER_HREF[ev.layer](ev.sampleIds);
              return (
                <Link
                  key={ev.layer}
                  href={href}
            ...
            {selectedDay && (
              <div className="pt-2 border-t" data-testid="calendar-day-actions">
                ...
                <div className="flex flex-wrap gap-1">
                  <Link
                    href="/fleet/transport/bookings/create"
                    className="text-[10px] px-2 py-0.5 rounded border border-border hover:bg-muted/50"
                    data-testid="calendar-action-create-booking"
                  >
                    حجز جديد
                  </Link>
                  <Link
                    href="/fleet/transport/ops-dashboard"
                    className="text-[10px] px-2 py-0.5 rounded border border-border hover:bg-muted/50"
                    data-testid="calendar-action-ops"
                  >
                    لوحة التشغيل
                  </Link>
                </div>

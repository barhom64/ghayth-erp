...
function SeverityCard({ label, count, color }: { label: string; count: number; color: string }) {
  const colorMap: Record<string, string> = {
    ...
    yellow: "bg-status-warning-surface border-status-warning-surface text-status-warning-foreground",
          ...
          <Tabs defaultValue="entities" dir="rtl">
            ...
            <TabsContent value="entities" className="mt-4 space-y-2">
              {entLoading ? (
                ...
                <p className="text-muted-foreground text-sm p-4">لا توجد كيانات</p>
              ) : (
                    ...
                    <Card key={e.id} className="overflow-hidden">
                      ...
                      {isExpanded && (
                        <CardContent className="border-t bg-muted/10 p-4 text-sm space-y-3">
                          ...
                          {e.approval && (
                            <div>
                              <p className="text-muted-foreground text-xs mb-1">سلسلة الاعتماد:</p>
            ...
            <TabsContent value="coverage" className="mt-4 space-y-4">
              {covLoading ? (
                <p className="text-muted-foreground text-sm p-4">جاري تحليل التغطية...</p>
              ) : (
                <>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      ...
                      <CardContent className="space-y-2">
                        {Object.entries(coverageByCategory).map(([cat, count]) => (
                          <div key={cat} className="flex items-center gap-3">
                            <span className="text-xs text-muted-foreground w-40 shrink-0">{categoryLabel(cat)}</span>
            ...
            <TabsContent value="notifications" className="mt-4 space-y-4">
              {notifLoading ? <div className="text-center text-sm text-muted-foreground py-8">جاري التحميل...</div> : <>
              ...
              {Object.entries(notifRegistry?.byDomain ?? {}).map(([domain, entries]: [string, any]) => (
                <Card key={domain}>
                  ...
                  <CardContent className="space-y-2">
                    {entries.map((e: any) => (
                      <div key={e.entityId} className="flex items-start gap-2 p-2 bg-surface-subtle rounded-lg">
            ...
            <TabsContent value="reports" className="mt-4 space-y-4">
              {reportLoading ? <div className="text-center text-sm text-muted-foreground py-8">جاري التحميل...</div> : <>
            ...
            <TabsContent value="print" className="mt-4 space-y-4">
              {printLoading ? <div className="text-center text-sm text-muted-foreground py-8">جاري التحميل...</div> : <>
              <div className="grid grid-cols-2 gap-3">
                ...
                <CardContent>
                  <div className="space-y-2">
                    ...
                    {(printRegistry?.templates ?? []).length === 0 && (
                      <p className="text-sm text-muted-foreground text-center py-4">لا توجد قوالب طباعة مسجلة</p>
                    )}
                  </div>
                </CardContent>
              </Card>
                  ...
                  {missLoading ? (
                    ...
                    <div className="flex flex-wrap gap-1">
                      ...
                      {(missing?.tablesWithoutEvents?.count ?? 0) === 0 && (
                        <p className="text-status-success-foreground text-sm">لا توجد فجوات — كل الجداول مغطاة</p>
                      )}
                    </div>
                  )}
                </CardContent>
                  ...
                  {missLoading ? (
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      {(missing?.domainsWithoutLifecycle?.count ?? 0) === 0 && (
                        <p className="text-status-success-foreground text-sm">كل النطاقات لديها آلة حالة</p>
                      )}
                    </div>
                  )}
                </CardContent>

...
const schema = z.object({
  ...
  costPrice: z
    .string()
    .optional()
    .refine((v) => !v || Number(v) >= 0, "سعر التكلفة يجب أن يكون صفر أو أكثر"),
  sellPrice: z
    .string()
    .optional()
    .refine((v) => !v || Number(v) >= 0, "سعر البيع يجب أن يكون صفر أو أكثر"),
  currentStock: z
    .string()
    .optional()
    .refine((v) => !v || Number(v) >= 0, "المخزون الحالي يجب أن يكون صفر أو أكثر"),
  minStock: z
    .string()
    .optional()
    .refine((v) => !v || Number(v) >= 0, "الحد الأدنى يجب أن يكون صفر أو أكثر"),
...
export default function WarehouseCreate() {
  ...
  const addProduct = useApiMutation("/warehouse/products", "POST", [
    ["warehouse-products"],
    ["warehouse-stats"],
  ]);

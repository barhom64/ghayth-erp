    ...
    <CreatePageLayout title="إضافة عميل جديد" backPath="/clients">
      ...
      <FormShell
        ...
        onSubmit={async (values) => {
          // Validate portal-account fields (outside the FormShell schema
          // because they live in local state, not the form data).
          setPortalPasswordErr(undefined);
          if (createPortalAccount) {
            if (!values.email) {
              toast({
                variant: "destructive",
                title: "يرجى إدخال البريد الإلكتروني لإنشاء حساب البوابة",
              });
              return;
            }
            if (portalPassword.length < 6) {
              setPortalPasswordErr("كلمة مرور البوابة يجب أن تكون 6 أحرف على الأقل");
              return;
            }
          }
      ...
      >
        <FormTextField name="name" label="اسم العميل / الشركة" required />
        <FormGrid cols={2}>
          <FormSelectField name="type" label="نوع العميل" options={TYPE_OPTIONS} />
          <FormSelectField name="classification" label="التصنيف" placeholder="بدون تصنيف" options={CLASSIFICATION_OPTIONS} />
        ...
        <FormSelectField name="source" label="مصدر العميل" placeholder="اختر المصدر" options={SOURCE_OPTIONS} />
        ...
        <div className="mt-2 border rounded-lg p-4 bg-status-info-surface">
            ...
            <div className="mt-3 space-y-2">
              ...
              <div className="space-y-1.5">
                <label className="text-sm font-medium">
                  كلمة المرور المؤقتة <span className="text-status-error-foreground">*</span>
                </label>
                <input
                  type="password"
                ...
                />
                {portalPasswordErr && (
                  <p className="text-xs text-status-error-foreground">{portalPasswordErr}</p>
                )}
              </div>
            </div>
          )}

...
export interface EmployeeCreateFormProps {
  /** Created employee row (incl. userAccount) on success — embedded/drawer mode. */
  onCreated?: (created: any) => void;
  /** Operator cancelled — embedded/drawer mode. */
  onCancel?: () => void;
  /** Draft-recovery key — distinct per host (page vs inline drawer). */
  draftKey?: string;
  /** Embedded (drawer) mode: drop page chrome + wizard nav, disable nested
   *  inline-create (no recursive drawers), and return the row via onCreated
   *  instead of showing the page success view. */
  embedded?: boolean;
}

export function EmployeeCreateForm({ onCreated, onCancel, draftKey = "employees_create", embedded = false }: EmployeeCreateFormProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (embedded) onCreated?.(result);
      else setCreationResult(result as Record<string, any>);
    } catch (err) {
  ...
  const body = (
    <>
      ...
      {!embedded && <WizardStepNav form={form as unknown as Record<string, string>} fieldErrors={fieldErrors} />}
      <div className="mb-4">
        <CreationDateField />
      </div>
      ...
      <div className="flex justify-end gap-3 pt-6">
        <Button variant="outline" onClick={embedded ? () => onCancel?.() : () => setLocation("/employees")}>إلغاء</Button>
  ...
  return embedded ? body : (
    <CreatePageLayout title="إضافة موظف جديد" backPath="/employees">{body}</CreatePageLayout>
  );
}

/** Embedded wrapper for the AllowCreateDrawer — forces embedded mode so the
 *  drawer hosts the full employee form without page chrome or recursion. */
export function EmployeeCreateDrawerForm({ onCreated, onCancel }: { onCreated: (created: any) => void; onCancel: () => void }) {
  return <EmployeeCreateForm embedded onCreated={onCreated} onCancel={onCancel} />;
}

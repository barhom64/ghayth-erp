...
const schema = z
  .object({
    ...
    maxParticipants: z
      .string()
      .optional()
      .refine((v) => !v || Number(v) > 0, "السعة القصوى يجب أن تكون أكبر من صفر"),
          ...
          {TRAINING_TYPES.map((tt) => (
            <button
              ...
              className={cn(
                ...
                field.value === tt.value ? `${tt.color} ring-2 ring-offset-1` : "border-border hover:border-border",
              )}
            >
    ...
    <CreatePageLayout title="إضافة برنامج تدريبي" backPath="/hr/training">
      ...
      >
        ...
        <FormGrid cols={2}>
          ...
          <FormSelectField name="category" label="التصنيف" placeholder="اختر التصنيف" options={CATEGORY_OPTIONS} />
        ...
        <h3 className="text-sm font-semibold text-muted-foreground mb-3 flex items-center gap-2 mt-4">
          <Clock className="h-4 w-4" /> المدة والتواريخ
        </h3>
        <FormGrid cols={2}>
          <FormNumberField name="duration" label="المدة" placeholder="٠" min="0" />
          <FormSelectField name="durationUnit" label="وحدة المدة" options={DURATION_UNIT} />

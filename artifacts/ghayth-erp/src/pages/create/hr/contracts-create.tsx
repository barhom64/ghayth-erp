...
function HiddenAssignmentSync({ employees }: { employees: any[] }) {
  const { watch, setValue } = useFormContext();
  const employeeId = watch("employeeId");
  const selected = employees.find((e: any) => String(e.id) === employeeId);
  // Keep assignmentId in sync with employee selection for downstream submit
  if (selected && selected.assignmentId) {
    setValue("__assignmentId" as any, selected.assignmentId);
  }
  return null;
}
...
export default function ContractsCreate() {
  ...
  return (
    <CreatePageLayout title="عقد موظف جديد" backPath="/hr/contracts">
      ...
      <FormShell
        ...
        onSubmit={async (values) => {
          ...
          await createMut.mutateAsync({
            ...
            assignmentId: selectedEmp?.assignmentId || Number(values.employeeId),
            ...
            housingAllowance: values.housingAllowance
              ? Number(values.housingAllowance)
              : undefined,
      ...
      >
        <HiddenAssignmentSync employees={employees} />
        <FormGrid cols={2}>
          <FormEntitySelect name="employeeId" select={EmployeeSelect} label="الموظف" required />
          <FormSelectField name="contractType" label="نوع العقد" required options={CONTRACT_TYPE_OPTIONS} />

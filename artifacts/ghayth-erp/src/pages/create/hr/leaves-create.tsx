...
function calcDaysCount(startDate: string, endDate: string): number {
    ...
    <div className="mb-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {balances.slice(0, 4).map((b: any) => (
          <Card
            key={b.id || b.type}
            className={`border ${String(b.leaveTypeId) === leaveTypeId ? "border-status-info-surface bg-status-info-surface ring-1 ring-blue-200" : "border-border"}`}
          >
...
function DaysCountBanner({
  ...
  const selectedBalance = balances.find((b: any) =>
    String(b.leaveTypeId) === leaveTypeId || b.name === selectedType?.name,
  );
  return (
...
function ReliefOfficerPicker({ employees }: { employees: any[] }) {
  const { control } = useFormContext();
  return (
    <div className="space-y-1.5">
      <label className="text-sm font-medium">المكلّف بالعمل أثناء الإجازة</label>
      <Controller
        name="reliefOfficer"
        control={control}
        render={({ field }) => (
...
export default function LeavesCreate() {
  ...
  const leaveTypeOptions = leaveTypes.length > 0
    ? leaveTypes.map((lt: any) => ({ value: String(lt.id), label: lt.name }))
    : [{ value: "", label: "لا توجد أنواع إجازات معرّفة — يرجى إعدادها أولاً", disabled: true }];
  ...
  return (
    <CreatePageLayout title="طلب إجازة جديد" backPath="/hr/leaves">
      ...
      >
        ...
        <h3 className="text-sm font-semibold text-status-neutral-foreground">تفاصيل الإجازة</h3>
        <FormGrid cols={2}>
          <FormSelectField
            name="leaveTypeId"
            label="نوع الإجازة"
        ...
        <DaysCountBanner balances={balances} leaveTypes={leaveTypes} />
        ...
        <FormGrid cols={2}>
          <ReliefOfficerPicker employees={employees} />
          <FormTextField
            name="contactDuringLeave"
            label="رقم التواصل أثناء الإجازة"
            placeholder="05xxxxxxxx"
            description="للتواصل في حالات الطوارئ"
          />
        </FormGrid>

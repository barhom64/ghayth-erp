...
export interface OwnerFormProps {
  /** Called with the freshly-created owner row after a successful save. */
...
/**
 * النموذج الكامل الموحّد لإنشاء مالك عقار — يُشارَك بين الصفحة الكاملة
 * (`owners-create.tsx`) والإضافة السريعة من `PropertyOwnerSelect` عبر الدرج
 * الموحّد (`AllowCreateDrawer`، نوع "property-owner"). يملك حالته/تحقّقه/إرساله،
 * فالإنشاء الداخلي مطابق لإنشاء الصفحة — لا إضافة مبتورة (توجيه إبراهيم «أ»).
 */
export function OwnerForm({ onCreated, onCancel, draftKey = "properties_owners_create" }: OwnerFormProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const created = await apiFetch("/properties/owners", { method: "POST", body: JSON.stringify(payload) });
      clearDraft();

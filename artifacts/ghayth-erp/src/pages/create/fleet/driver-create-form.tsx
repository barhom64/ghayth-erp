...
export interface DriverCreateFormProps {
  /** Called with the freshly-created driver row after a successful save. */
...
/**
 * The unified driver-creation form body — shared by the full page
 * (`fleet/drivers-create.tsx`) and the inline `AllowCreateDrawer` opened from
 * `DriverSelect`. Owns its own state / validation / mutation / draft so an
 * inline create is identical to a page create — the full form (KSA license
 * identity + employee link), no truncated quick-add.
 */
export function DriverCreateForm({ onCreated, onCancel, draftKey = "fleet_drivers_create", showAttachments = true }: DriverCreateFormProps) {

...
export default function OvertimeDetail() {
  ...
  const kpis = [
    ...
    { label: "المبلغ", value: formatCurrency(Number(item.totalAmount || 0)), icon: DollarSign, color: "text-status-success-foreground bg-status-success-surface" },
  ];
          ...
          <Card className="border-0 shadow-sm">
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                ...
                <div>
                  <p className="text-muted-foreground">تاريخ العمل الإضافي</p>
                ...
                <div>
                  <p className="text-muted-foreground">فترة الرواتب</p>
                ...
                {item.rejectReason && (
                  <div className="col-span-full">
                    ...
                    <p className="font-medium text-status-error-foreground">{item.rejectReason}</p>
                  </div>
                )}
              </div>
            </CardContent>
          ...
          <Card className="border-0 shadow-sm">
            ...
            <CardContent>
              <div className="space-y-3 text-sm">
                ...
                <div className="flex justify-between">
                  <span className="text-muted-foreground">معامل الضرب</span>
                  <span>×{Number(item.multiplier || 1.5).toFixed(2)}</span>
                </div>
                <hr className="border-border" />
                <div className="flex justify-between font-bold text-base">
                  <span>إجمالي التعويض</span>
                  <span className="text-status-success-foreground">{formatCurrency(Number(item.totalAmount || 0))}</span>
                </div>
              </div>
            </CardContent>
          </Card>

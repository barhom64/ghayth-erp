...
export default function AdminPolicyEngine() {
  ...
  const roleStrategyColumns: DataTableColumn<any>[] = [
    ...
    { key: "description", header: "الوصف", render: (r: any) => <span className="text-xs text-muted-foreground">{r.description}</span> },
  ];
      ...
      <PageStateWrapper isLoading={isLoading && !audit} error={auditError} onRetry={refetchAudit}>
        <div className="space-y-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Card className={critical > 0 ? "bg-status-error-surface" : "bg-status-success-surface"}>
            ...
            <Card className={critical > 0 ? "bg-status-error-surface" : "bg-status-success-surface"}>
              <CardContent className="p-4 text-center">
                <p className="text-2xl font-bold text-status-error-foreground">{critical}</p>
            ...
            <Card>
              <CardContent className="p-4 text-center">
                ...
                <p className="text-xs text-muted-foreground">قواعد فصل المهام</p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4 text-center">
                <p className="text-2xl font-bold">{sensitiveOps.length}</p>
                <p className="text-xs text-muted-foreground">عمليات حساسة</p>
              </CardContent>
            </Card>
          </div>
          ...
          {violations.length === 0 && audit && (
            <Card className="border-status-success-surface bg-status-success-surface">
              <CardContent className="p-6 text-center text-status-success-foreground">
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                {separationRules.map((rule: any, i: number) => (
                  <div key={i} className="p-2 bg-surface-subtle rounded border text-sm flex items-start gap-2">
                    <ShieldAlert className="w-4 h-4 text-status-warning mt-0.5 shrink-0" />
                    <div>
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                {sensitiveOps.map((op: any, i: number) => (
                  <div key={i} className="p-2 bg-surface-subtle rounded border text-sm">
                    <div className="flex items-center justify-between">
                      <span className="font-mono text-xs">{op.permission}</span>
                      <div className="flex gap-1">
                        {op.requiresDualApproval && <Badge variant="outline" className="text-status-error-foreground">موافقة ثنائية</Badge>}

...
const approvalTypeConfig: Record<string, { label: string; icon: any; color: string; href: string }> = {
  ...
  loan: { label: "طلب سلفة", icon: Wallet, color: "text-status-info bg-status-info-surface", href: "/hr/loans" },
  overtime: { label: "وقت إضافي", icon: Timer, color: "text-cyan-500 bg-cyan-50", href: "/hr/overtime" },
  exit: { label: "نهاية خدمة", icon: LogOut, color: "text-status-error bg-status-error-surface", href: "/hr/exit" },
};
              ...
              <Link key={`${a.type}-${a.id}`} href={cfg.href}>
                <div className="flex items-center gap-3 p-2.5 rounded-lg hover:bg-surface-subtle transition-colors cursor-pointer">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-status-neutral-foreground truncate">{cfg.label} - {a.employeeName}</p>
                    <p className="text-xs text-muted-foreground">{a.title}</p>
                  </div>
                  <span className="text-xs text-muted-foreground">{a.createdAt ? formatTimeAgo(a.createdAt) : ""}</span>
                </div>
              </Link>
            );
          })}

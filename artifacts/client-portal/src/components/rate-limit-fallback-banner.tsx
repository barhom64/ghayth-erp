...
const STORAGE_KEY = "portal_ratelimit_fallback_dismissed_at";
...
export function RateLimitFallbackBanner() {
  const { data: erpUser } = useQuery<ErpUser | null>({
    queryKey: ["portal", "erp-auth-me", "ratelimit-banner"],
  ...
  const { data } = useQuery<SystemHealth | null>({
    queryKey: ["portal", "admin", "system-health", "ratelimit-banner"],

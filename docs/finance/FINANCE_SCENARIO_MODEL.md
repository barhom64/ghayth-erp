...
|---|---|---|
| **1** | السجلّ المركزي + الـ Resolvers (حقول/حساب/مركز تكلفة/أثر/مهمة) + الاختبارات + التوثيق + Field Audit | ✅ مُنفَّذ |
| **2** | `FinanceOperationContextPanel` يعرض التوجيه المحاسبي/الأثر/المهمة من السجلّ (`TARGET_HINTS`)، و`AllocationTarget` صار ألياس لـ `FinanceTarget` (مصدر واحد للقائمة) | ✅ مُنفَّذ |
| **3** | اختبارات تثبت أن `TARGET_HINTS` (غرض الحساب + الرسملة) = `deriveSpecializedAccount`، وأن المهمة المستقبلية تطابق `deriveOperationalEffectHint` — يُغلَق التطابق برمجيًا | ✅ مُنفَّذ |
| **4** | فصل `documentStatus`/`paymentStatus`/`postingStatus` (`status-model.ts`)، وحذف «الحالة» المختلطة و«تم الدفع» الميتة من نموذج المصروف، مع اختبارات القبول | ✅ مُنفَّذ (FE-first) |
| **5** | تنظيف نموذج المصروف: حذف «الجهة المرتبطة» القديمة (تُشتق من السيناريو)، و«التصنيف الضريبي» المكرّر، والقيد المتوقع ذي الرموز الثابتة | ✅ مُنفَّذ |
| **6** | نموذج السندات يعيد استخدام النموذج عبر `deriveRelatedEntity` المشترك، وتُحذف منتقيات الجهة المكرّرة | ✅ مُنفَّذ |
| — | (مستقبلي) هجرة قاعدة البيانات لأعمدة الحالة الثلاثة الفعلية + تعميم على الفواتير وشاشات الإدخال | لاحقًا |

## ملاحظة على المرحلة 4 (FE-first)

الخادم اليوم يخزّن المصروف بعمود `status` واحد + `isPaid`، وكلاهما كان بلا أثر
على الترحيل (القيد يُرحَّل ويخرج المال بصرف النظر عنهما). لذلك نُفِّذ الفصل
**على مستوى الواجهة والنموذج** أولًا: `status-model.ts` هو مصدر الحقيقة
للمحاور الثلاثة وقواعدها (مُختبَرة)، و`mapJournalStatus` يترجم حالة الخادم
الواحدة إلى محورَي العرض. هجرة المخطط لأعمدة فعلية ثلاثة مؤجَّلة (انظر الخارطة).

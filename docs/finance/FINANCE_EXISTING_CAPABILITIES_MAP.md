...
2. **المصروفات والسندات** — ربط الدفع بالحساب + حقل «ربط بـ» + توحيد الإنشاء عبر `FinanceOperationContext`. ✅ (PR-2/3/4 + PR-5 wave-1)
3. **الفواتير والمشتريات** — تعميم المكوّن + بنود من الكتالوج. (جزئياً #1519؛ المُحلِّل المركزي مُستخدَم في كل تدفّقات الإنشاء)
4. **الأصول والمخزون والعهد** — تعميم المكوّن + استنتاج CC. (المُحلِّل المركزي مُستخدَم؛ إنشاء العهدة يمرّ عبر `FinanceOperationContext` مع تحقّق دفع↔حساب اختياري)
5. **القوائم والتقارير** — توحيد الفلاتر + بوابة تقارير. ✅ (مجموعتا التقارير مدموجتان في مجموعة واحدة)
6. **القائمة الجانبية** — إعادة التصنيف حسب دورة العمل (13 مجموعة). ✅ (إعادة تنظيم حاويات فقط: لا تغيير مسار، الإعدادات آخراً، التقارير مدموجة)
...
| #2 — منع `code.startsWith` كمنطق حسابات أساسي | قاعدة lint `account-code-startswith-in-finance-page` (ratchet، بدأ 18 ← الآن 15) تمنع أي منطق بادئة-كود جديد في صفحات/مكوّنات المالية | ✅ مُفعَّلة (مُنتقِيا المصروف/السند نُقلا إلى `isMoneyAccount` المركزي؛ السقف ينزل مع كل ترحيل) |
...
> مارًّا عبر السياق والخدمات المركزية.
>
> **ملاحظة الموجة 3/4 (الواجهة):** ترحيل صفحات الفواتير/الأصول من `code.startsWith`
> ومن `LineAllocationPanel` الخام إلى `finance-account-usage` + `AllocationTargetSelect`
> تغييرٌ بصري لكل-بند يتطلّب تحقّقًا في المتصفّح؛ يُنفَّذ تدريجياً بإسقاط أساس
> الـratchet مع كل صفحة محوّلة، فلا انحدار في القدرات.

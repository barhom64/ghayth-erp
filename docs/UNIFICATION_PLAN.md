...
| P1.2 | `FormShell` + field components | `components/form-shell.tsx` (433 line) | 223/431 صفحة (51.7%) — كل صفحات create فعلت الانتقال (76/76) + 10 shared components fan out across detail/approval pages (entity-comments, entity-selects QuickCreateDialog, detail-edit-delete-actions InlineEditCard, approval-actions ApprovalActions, …) |
...
| `useState-in-create-edit-page` (يفرض FormShell) | صفحات edit/details فقط (كل create انتهت) | لا — يبقى pattern في صفحات edit |
| `unscoped-status-badge` (`<Badge>` خام لـ status) | غير مُقاس | لا — يحتاج جرد أولًا |

> **تحديث**: قاعدة `manual-form-instead-of-formshell` تم تفعيلها كقاعدة صلبة (hard rule) في `scripts/src/lint-patterns.mjs` بعد إكمال نقل كل صفحات `pages/create/` (75 صفحة) إلى FormShell + zod. الـ hooks القديمة `use-auto-draft.ts` و `use-field-errors.ts` تم حذفها من الـ tree. أي إعادة استيراد لـ `useAutoDraft` أو `useFieldErrors` تحت `src/{pages,components,hooks,lib}` تُسقط الـ build.

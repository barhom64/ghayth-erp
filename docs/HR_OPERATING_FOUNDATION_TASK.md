...
| 10 | التقييم وإشارات الأداء | 🟡 **Engine + storage مبني** (HR-006): 6 أبعاد + composite + trend + rationale. بقي: cron + UI + Risk/Promotion/Burnout engines (المؤشرات الـ3 المنفصلة في §G من #1799). | بناء `employeeScoringEngine` ✅ + Risk/Promotion/Burnout لاحقًا |
...
| HR-006 | **الأولوية #10 — Employee Scoring Engine**. Migration 272 يُنشئ `employee_scores` (UNIQUE assignment×scope×periodKey + 6 dimension columns + trend + JSONB rationale/weightsUsed/rawCounters + indexes للأرشيف والترتيب). **محرك جديد** `lib/employeeScoringEngine.ts` يحسب composite من 6 أبعاد (Discipline 20% / Activity 15% / Productivity 35% / Quality 15% / Manager 10% / Development 5%) — كل counter يقرأ من جدول موجود (employee_violations + attendance + audit_logs + project_tasks + hr_performance_evaluations + training_enrollments). يدعم weekly/monthly/quarterly مع ISO-week math. Trend (-1/0/+1) محسوب من السجل السابق. Rationale عربي لكل بُعد. UPSERT idempotent بـ ON CONFLICT. Manager dimension ترجع 60 (محايد) عند غياب تقييم. 27/27 smoke tests خضراء. بقي: cron يومي/أسبوعي + UI داخل ملف الموظف. | 2026-06-08 |

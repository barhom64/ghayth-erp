...
| HR-005 | **الأولوية #7 — التتبع الميداني (الأساس + ingestion)**. Migration 271 يُنشئ `field_tracking_points` (lat/lng/accuracy/speed/heading/altitude/battery/deviceId/source/task+trip+visit refs/capturedAt+createdAt) + index على (assignmentId, capturedAt). endpoint `POST /hr/attendance/field-ping` يستقبل النقاط ويفرض `trackingFrequencySeconds` من المحرك (السائق 30s، الميداني 300s، الإداري/المدير/التنفيذي 0 ⇒ مرفوض بـ 403)، مع throttle 80% tolerance يرجع 202 no-op بدل الخطأ. endpoint `GET /hr/attendance/field-track` بنمطين: breadcrumb (موظف ليوم كامل) و live (DISTINCT ON أحدث نقطة لكل موظف). 17/17 smoke tests خضراء. بقي: ربط صفحة `field-tracking.tsx` بالـ endpoint الجديد بدل `/hr/attendance`. | 2026-06-08 |

...
| HR-004 | **الأولوية #6 — wiring 2/2**. (أ) ربط `POST /hr/check-out` بنفس النمط — early-departure violation + deduction يحرسهما `autoDeductionEnabledCheckout`. (ب) ربط `autoViolationEngine.runAutoDetection` بـ `resolveBatch` (memoized) ⇒ الـ cron الليلي يحذف incidents الفئات المُعفاة قبل INSERT INTO `employee_violations` و قبل `ensureInquiryMemoForViolation`. `result.detected` يبقى يعكس الـ raw count قبل الفلترة للـ audit. fallback آمن: فشل engine = legacy behavior + error log. 12/12 wiring smoke tests خضراء، 23/23 existing autoViolation tests لا تزال خضراء. | 2026-06-08 |

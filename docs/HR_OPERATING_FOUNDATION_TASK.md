...
| 3 | نموذج "ماذا يظهر لمن ومتى" | 🟢 **مكتمل** (HR-014) — `/admin/effective-permissions` viewer + deep-link من ملف الموظف. | UI viewer للـ Effective Permissions ✅ |
...
| 7 | التتبع حسب السياسة | 🟢 **مكتمل** (HR-005 + HR-015) — Backend ingestion + Frontend breadcrumb + KPIs مدمجة في `/hr/field-tracking`. | ✅ |
...
| 10 | التقييم وإشارات الأداء | 🟢 **مكتمل end-to-end** (HR-006, HR-007, HR-009, HR-014). engines + cron + PerformanceWidget في overview tab. | scoring + signals + cron + UI ✅ |
| 11 | صندوق الأعمال والجدولة | 🟢 **مكتمل** (HR-010 + HR-016) — Services Catalog + Unified `/my/work-queue` يجمع approvals + tasks + notifications + threads في feed واحد. | ✅ |
...
| HR-014 | **الأولوية #10 + الأولوية #3 + §K integration tests**. (أ) **Scoring/Signals widget في overview tab**: `GET /employees/:id` يُضيف `latestScore` (آخر صف monthly من `employee_scores` + 6 dimensions + trend + rationale) و `activeSignals` (الصفوف غير المؤكدة من `employee_signals` آخر 90 يومًا، مرتبة critical→high→medium→low). الـ `PerformanceWidget` في الفرونت يعرض composite score مع لون حسب العتبة (≥85 أخضر، ≥70 أزرق، ≥50 برتقالي، <50 أحمر) + trend arrow + 6 dimensions grid + أول 3 إشارات نشطة بألوان severity. عدد الـ tabs يبقى 14 (الـ widget داخل overview). (ب) **Effective Permissions viewer** (`/admin/effective-permissions`): صفحة جديدة تستهلك `GET /admin/users/:id/effective-permissions` (موجود مسبقًا) — تجميع حسب module، فلتر بحث، Arabic translation للـ scopes والـ actions، overrides تُعرض منفصلة بـ deny-wins styling. يفتح من ملف الموظف 360 → روابط الأدوار → "عرض الصلاحيات الفعلية" مع `?userId=N`. nav entry تحت إعدادات HR. (ج) **§K integration tests**: `tests/integration/hrAttendancePolicyByCategory.dynamic.test.ts` (gated on DATABASE_URL) — 4 سيناريوهات: عامل يُخصم، مدير لا يُخصم (core #1799 §F.6 rule)، per-company override يتغلب على system seed، uncategorised assignment يرجع لـ legacy default بلا crash. 22/22 unit smoke tests خضراء. **يُغلق #10 و #3 من 🟡 إلى 🟢**. | 2026-06-08 |
| HR-015+016+017 | **إكمال المتبقي الثلاثي**. (أ) **HR-016 — Unified Work Queue** (`/my/work-queue`): صفحة جديدة تجمع 4 مصادر "ما ينتظرني" (HR approvals من `/my-space` + tasks من `/tasks` + notifications من `/notifications` + unread threads من `/inbox/threads`) في feed موحد بـ 4 tabs + counts + deep-link bar للشاشات الأصلية. تحل التكرار المرصود في §5 من تقرير الـ audit. (ب) **HR-015 — Field Tracking breadcrumb**: ترقية `pages/hr/field-tracking.tsx` بإضافة `FieldBreadcrumbSection` تستهلك `/hr/attendance/field-track` مع date+assignment picker + خريطة polyline + KPIs (نقاط/مسافة/سرعة/بطارية). البيانات في `field_tracking_points` كانت تُكتب بدون أحد يقرأها — الآن مرئية. (ج) **HR-015 — Attendance Categories admin** (`/admin/attendance-categories`): 3 endpoints جديدة في `routes/org.ts` (GET employee-categories + GET/POST/DELETE attendance-policies-per-category مع UPSERT keyed على companyId+categoryKey + zod bounds). الصفحة تعرض الـ 6 system categories (read-only) + جدول overrides + form ينشئ/يعدّل أي override بدون SQL مباشر. (د) **HR-017 — Custodies Model Boundaries doc** (`docs/audit/CUSTODIES_MODEL_BOUNDARIES.md`): يحسم "3 نماذج العهد" المرصود في §5 — يثبت أنها 3 طبقات متكاملة (warehouse_stock_serials = أين؟ / employee_assets = مع من؟ / subsidiary_accounts = كم تكلّفنا؟)، ليست متنافسة. يوضح متى تُستخدم كل طبقة + 3 risks متبقية للمستقبل. 26/26 unit smoke tests خضراء + 47/47 existing HR smoke tests لا تزال خضراء. **يُغلق #7 (frontend) و #11 (unified inbox) و R3 من تقرير الـ audit. التكرار في العهد تم توثيقه**. | 2026-06-08 |

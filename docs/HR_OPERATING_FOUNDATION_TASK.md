...
| HR-008 | **§B (نموذج المؤسسة التشغيلي) — الدفعة الأساسية**. Migration 274 يُنشئ 5 جداول + يضيف 3 أعمدة: (1) `legal_entities` — multi-CR/VAT per company. (2) `branches.legalEntityId` nullable. (3) `positions` كـ catalog مع 9 system positions seeded (staff/specialist/senior/team_lead/supervisor/assistant_manager/manager/general_manager/executive). (4) `employee_assignments.positionId + legalEntityId` (nullable). (5) `teams` (داخل departments) + `employee_team_memberships`. (6) `committees` (cross-dept، مع chair + startDate/endDate) + `employee_committee_memberships` مع isVoting. (7) `employee_project_assignments` مع `allocationPercent NUMERIC(5,2) CHECK > 0..100` + `costCenterId` للتقسيم على المشاريع، UNIQUE (assignment, project, startDate) للسماح بإعادة التعيين بعد فجوة، index جزئي على endDate IS NULL للتجميع السريع للتكاليف النشطة. كل التغييرات additive + @rollback مرتّب dependency. 18/18 smoke tests خضراء. | 2026-06-08 |

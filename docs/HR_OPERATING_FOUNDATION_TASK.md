...
| HR-007 | **الأولوية #10 §G — Risk/Promotion/Burnout Signals**. Migration 273 يُنشئ `employee_signals` (UNIQUE assignment×signalType×scope×periodKey + 4 severities CHECK + reasons JSONB array + acknowledgement support). **محرك جديد** `lib/employeeSignalsEngine.ts` بـ 3 detection engines: Risk (composite<50 / discipline<50 / productivity<40 / trend=-1 لفترتين)، Promotion (composite≥85 + discipline≥90 + productivity≥80 + quality≥80 + trend=+1، ≥3 معايير)، Burnout (productivity≥70 مع discipline<65 / drop≥15 / 1200min OT بلا إجازة). UPSERT idempotent مع acknowledgement reset عند escalation الـ severity فقط. 32/32 smoke tests خضراء. | 2026-06-08 |

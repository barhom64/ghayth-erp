...
Generated: 2026-05-20T14:45:56.190Z
...
| Files with ≥1 hand-rolled hit | 83 |
| Total hand-rolled hits | 2288 |

## By Category

| Class | Meaning | Files | Hits |
|---|---|---:|---:|
| **A** | Safe — mechanical `buildScopedWhere` swap | 58 | 1878 |
...
| `artifacts/api-server/src/routes/events.ts` | A | 2 | 0 | 0 | aliased fraction 0% (0/2) — predominantly plain `"companyId" = $N`; mechanical buildScopedWhere swap candidate (per-handler review still needed for the 0 aliased hits) |
...
### `artifacts/api-server/src/routes/events.ts` (A, 2 hits)

- L96: `let where = \`"companyId" = $1\`;`
- L185: `WHERE "companyId" = $1 AND "createdAt" >= NOW() - ($2 || ' days')::interval`

...
#728/#729, #730, #731, #732, #734, #736. Post-merge runtime probe note: the
`/financial-requests` 500 (a pre-existing `workflow_requests` schema drift) is
fixed separately by PR #746; the other four Wave-9 endpoints need a Replit
rebuild + re-probe (their contract code is correct on `main`).
...
1. **Runtime-verify the six merged fix waves** — #728/#729, #730, #731, #732,
   ...
   against a live ledger; the `/financial-requests` 500 found post-merge is
   addressed by PR #746 (`workflow_requests` schema-drift reconciliation).

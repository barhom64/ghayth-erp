...
> **الفئة التاسعة — `UPDATE … SET <col>` على عمود غير موجود (شقيق `check:insert-columns` على جانب التحديث) (migration 284):** أعمدة جملة `SET` هي حتمًا أعمدة الجدول المُحدَّث (لا يمكن تسميتها بـalias)، فالفحص دقيق ومرتبط بالجدول. كاشف `check:update-columns` مسح **1501** إسناد `SET` فكشف **9 أعمدة يكتبها الكود ولم تُضَف للجدول قَطّ** — نفس جذر migration 279 لكن مُعمَّمًا خارج `updatedAt`، كلّها تنهار بـ`column … does not exist` فور بلوغ مسارها. أُصلِحت دفعةً بهجرة إضافية واحدة (`ADD COLUMN IF NOT EXISTS`، غير هدّامة): `updatedAt` على `clients`/`documents`/`employee_assignments`؛ `approvedBy`+`approvedAt` على `employee_commission_calculations` (**اعتماد العمولة كان ينهار**، والرفض يعمل)؛ `deletedAt` على `gov_integration_links` (الـ`SELECT` فوق الـ`UPDATE` يُرشِّح `deletedAt IS NULL` أيضًا، **فكامل مسار الحذف الناعم كان ميتًا**)؛ `paidAmount` على `purchase_orders` (تراكم سداد AP-aging)؛ `lastSentAt` على `scheduled_reports` (كرون التقارير)؛ `totalSpend` على `suppliers` (عدّاد مكتوم بـ`.catch` «قد لا يوجد بعد»). مُثبت حيًّا: الأشكال التسعة تُنفَّذ، و`SELECT … deletedAt IS NULL` على `gov_integration_links` يعمل. الكاشف الآن **= 0 انتهاك** (436 جدول)، مُختبَر guard-the-guard. أداة تشخيص يدوية لنفس سبب أخواتها.

> **الفئة العاشرة — `INSERT … ON CONFLICT (cols)` بلا قيد فريد مطابق (migration 285):** جملة `ON CONFLICT (<cols>)` تحتاج قيدًا فريدًا/إقصائيًا تطابق أعمدته `<cols>` تمامًا، وإلا تُرفض وقت التشغيل بـ`there is no unique or exclusion constraint matching the ON CONFLICT specification` — لا يلتقطها أي فحص ثابت لأن كل الأعمدة موجودة. كاشف `check:on-conflict-targets` (محدود بقالب SQL واحد لمنع اقتران INSERT بـON CONFLICT من جملتين مختلفتين؛ يتخطّى الفهارس الجزئية/التعبيرية بأمانة) مسح ON CONFLICT في الشيفرة فكشف واحدًا حقيقيًا: **`fleet_alerts`** يُحدِّث-أو-يُدرج `ON CONFLICT ("companyId", type, "relatedType", "relatedId")` بينما الجدول **بلا أيّ فهرس فريد** على هذه الرباعية، فكل إعادة حساب للتنبيهات تنهار والجدول **يبقى فارغًا في كل البيئات** (الكاتب الوحيد لم ينجح قَطّ — أُثبت حيًّا). الإصلاح: فهرس فريد مطابق (غير جزئي، لأن `ON CONFLICT` بلا `WHERE` لا يستنتج فهرسًا جزئيًا)، مع dedup دفاعي. مُثبت حيًّا: الـupsert ينجح الآن (إدراج ثم upsert لنفس الرباعية ⇒ `DO UPDATE`). الكاشف **= 0 انتهاك** (86 هدفًا مُتحقَّقًا، 13 متخطّى لفهارس جزئية/تعبيرية)، مُختبَر guard-the-guard. أداة تشخيص يدوية لنفس سبب أخواتها.

> **خلاصة مراجعة المسارات الكتابية — حزمة عقود-المخطط (5 كواشف، أمرٌ واحد):** الفئات 5/7/8/9/10 هي **عقدٌ بين الكود والمخطط الحيّ** يُخالَف وقت التشغيل فقط (SQL سليم نحويًّا لكنه يناقض المخطط، فلا تراه اختبارات الوحدة). جُمِعت كواشفها الخمسة في أمر واحد `pnpm check:write-path-contracts` (يشغّل `check:insert-columns` + `check:required-columns` + `check:update-columns` + `check:constraint-literals` + `check:on-conflict-targets` بـ`--strict`). على قاعدة على **رأس الترحيلات**: **الخمسة خضراء**. الكلّ يشترط قاعدة على رأس الترحيلات (قاعدة على ما دونها تُنتج إيجابيات كاذبة — كما لوحظ حين كانت ترحيلتان غير مطبّقتين)، ولهذا لا يُوصَل في `guard.sh`: workflow الحارس يُحمِّل dump المخطط ثم **يَسِم الترحيلات «مطبّقة» دون تشغيلها**، فالقاعدة دون الرأس. التفعيل الذاتي مؤجَّل لمسار CI يُقلع قاعدة على رأس الترحيلات (مهمة بنية تحتية منفصلة، لا تُخاطَر بحارس الدمج المطلوب).
>
> **الفئات البنيوية المتحقَّقة نظيفة (لا عطل):** عدد الأعمدة مقابل القيم في `INSERT … VALUES`/`… SELECT` (706 + الباقي = 0 عدم تطابق)؛ كتابات قاعدة بيانات معلَّقة بلا `await` (كلّها fire-and-forget مقصودة بـ`.catch`)؛ كتابات «مسرّبة من المعاملة» (`rawExecute`/`rawQuery` داخل `withTransaction` تنضمّ تلقائيًّا للمعاملة عبر `AsyncLocalStorage`، تصميم سليم). هذا البُعد مغطّى ومُتحقَّق.

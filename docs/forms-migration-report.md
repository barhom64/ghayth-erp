...
| Backend coverage by UI (Phase C) | **503/1321 (38.1%)** — 818 endpoints have no frontend caller | ⚠️ report-only — `pnpm run audit:wiring` lists them grouped by domain. Many are intentionally backend-only (cron triggers, internal probes) but the list is the canonical "what features does the server have that no UI exposes" surface |

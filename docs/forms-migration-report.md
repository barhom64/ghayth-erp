...
| Backend coverage by UI (Phase C) | **723/1321 (54.7%)** — 598 endpoints have no frontend caller (down from 818 after regex fix in extractFrontendCalls revealed 236 hidden callers). Phase D underway: `/finance/cost-centers` page added (5-endpoint CRUD module — 3 statically-resolved, 2 dynamic) | ⚠️ report-only — `pnpm run audit:wiring` lists them grouped by domain |

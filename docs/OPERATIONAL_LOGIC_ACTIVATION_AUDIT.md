...
### 7.3 — الحل المعتمد: حسابات فرعية ديناميكية لكل كيان («نظام قوي قابل للتحكم»)
بدل أكواد ثابتة، تبنّينا رؤية المالك: **كل مركبة عند تسجيلها يُفتح لها تلقائيًا حساب فرعي قابل للترحيل** تحت الحساب الرئيسي المناسب (وقود/صيانة/إهلاك)، يحمل بيانات المركبة، وقابل للتعديل من `/finance/subsidiary-accounts`. أُعيد استخدام البنية الموجودة (`subsidiary_accounts` + `createSubsidiaryAccountsForEntity`) لا بناء جديد.

**ما نُفِّذ (رحلة الأسطول — مُثبَتة 9/9):**
- `253_subsidiary_accounts_fleet_entity_types.sql`: توسيع قيد `entityType` ليقبل `vehicle`/`driver` — كان يمنع إنشاء الحسابات الفرعية للمركبة/السائق صامتًا.
- `createSubsidiaryAccountsForEntity` (vehicle): يُنشئ الآن `fuel`(5510)/`maintenance`(5520)/`depreciation`(5710) + custody(1113) كأوراق فرعية لكل مركبة.
- `fleetEngine.postTripCompletionGL`: يُرحّل الوقود/الإهلاك على **حساب المركبة نفسها** (fallback: `accounting_mappings` ثم كود COA القياسي الصحيح — تصحيح 5200/1100 الخاطئين → 5510/5710/5140/1111).

**إثبات حيّ:** المركبة #1 حصلت على `5510-0001`(وقود)/`5520-0001`(صيانة)/`5710-0001`(إهلاك)؛ ورحلتها رحّلت الوقود على `5510-0001` متوازنًا (تتجمّع تحت الأب للتقارير المُجمّعة).

**متابعة (نفس النمط):** تعميم الحسابات الفرعية على Umrah (إيراد لكل وكيل/موسم — البنية موجودة عبر `resolveRevenueAccount`) وPayroll، ثم رحلات HR/عمرة الكاملة (#1609). السائق/العميل/المورد يملكون النمط ذاته.

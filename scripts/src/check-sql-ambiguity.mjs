...
// Split a single query scope into its independent set-operation branches
// at top-level UNION / UNION ALL / INTERSECT [ALL] / EXCEPT [ALL]. Each
// branch of a set operation (and the anchor vs recursive branch of a
// recursive CTE) has its OWN FROM list: a single-table anchor such as
// `SELECT id FROM cost_centers` must NOT be analysed together with a later
// self-joining recursive branch, or the anchor's bare columns get
// false-flagged as ambiguous. `extractScopes` has already replaced nested
// subqueries with `()`, so any set-op keyword still present in a scope is a
// genuine top-level branch boundary.
export function splitSetOpBranches(scope) {
  return scope.split(/\b(?:UNION|INTERSECT|EXCEPT)\b(?:\s+ALL\b)?/gi);
}
...
export function findAmbiguousRefs(scope, tableColumns) {
  ...
  for (const col of shared) {
    ...
    while ((m = qre.exec(scope)) !== null) {
      ...
      if (/\bas\s+$/i.test(before)) continue; // `AS "col"` alias definition
...
export function analyzeBody(body, tableColumns, fragMap) {
  ...
  for (const stmt of splitStatements(ambSql)) {
    for (const scope of extractScopes(stmt)) {
      for (const branch of splitSetOpBranches(scope)) {
        for (const f of findAmbiguousRefs(branch, tableColumns)) {
  ...
  for (const stmt of splitStatements(missSql)) {
    for (const scope of extractScopes(stmt)) {
      for (const branch of splitSetOpBranches(scope)) {
        for (const f of findMissingQualifiedColumns(branch, tableColumns)) {

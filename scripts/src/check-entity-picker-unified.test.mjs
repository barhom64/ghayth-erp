...
test("لا مخالفة على مكوّن في الأساس", () => {
  assert.deepEqual(
    violationsFrom([{ name: "supplier-item-picker.tsx", content: "// bespoke" }]),
    [],
  );
});
...
test("الأساس يضم الخمسة المنفصلة المعروفة", () => {
  for (const n of ["supplier-item-picker.tsx", "umrah-group-picker.tsx", "journal-template-picker.tsx", "location-kind-picker.tsx", "map-location-picker.tsx"]) {
    assert.ok(BASELINE.has(n), `يجب أن يكون ${n} في الأساس`);
  }
  assert.equal(BASELINE.size, 5);
});

...
test("الأساس = المتمايزة عمدًا فقط (بعد توحيد supplier-item B1 + journal-template B2)", () => {
  for (const n of ["umrah-group-picker.tsx", "location-kind-picker.tsx", "map-location-picker.tsx"]) {
    assert.ok(BASELINE.has(n), `يجب أن يكون ${n} في الأساس`);
  }
  assert.ok(!BASELINE.has("supplier-item-picker.tsx")); // وُحِّد B1
  assert.ok(!BASELINE.has("journal-template-picker.tsx")); // وُحِّد B2

...
-   **Frontend Runtime Audit (2026-05-07, Task #185 — supersedes the matrix below)**: Real headless-Chromium walk of every frontend route via `scripts/src/runtime-audit.cjs` (run with `pnpm run audit:runtime`). **233 PASS / 69 FAIL / 71 SKIP across 373 routes** on 5 axes (A1 render, A2 data fetch, A3 primary CTA, A4 navigation, A5 create/edit page smoke). All 69 FAILs are A5 `form=0/save=Y` on create/edit pages — the probe undercounts custom shadcn fields (`<Combobox>`, `<DatePicker>`, etc.) that don't expose native form elements until first interaction; reported as FAIL anyway so the result stays auditable. 71 SKIPs are `:id` routes whose list API returned 404 or was empty. Raw JSON: `audit/runtime-audit-results.json`. Full report: `FRONTEND_RUNTIME_AUDIT.md`.

...
-   **Guard suite (`pnpm run guard`)**: 6-step pre-merge gate run by `.github/workflows/guard.yml` on every PR. Blocks merge on failure. Steps:
    ...
    7. **sql-ambiguity**: `pnpm run check:sql-ambiguity` — flags any bare (unqualified) column reference in a multi-table JOIN/EXISTS rawQuery where that column exists on 2+ of the joined relations (the `column reference "status" is ambiguous` 500 class). Live-schema-backed (DB-gated like schema-drift/ghost-rows); pure-logic fixtures in `check-sql-ambiguity.test.mjs` run unconditionally. Allowlist: `scripts/sql-ambiguity-allowlist.txt`.

...
//
// This suite is intentionally broad and low-flake: it validates the final-user
// shell of critical Arabic/RTL routes, catches real runtime errors, and blocks
// technical placeholders from leaking into the Arabic UI. It complements deeper
// per-domain E2E journeys rather than replacing them.
...
async function expectArabicRtlOperationalPage(pageText: string, route: string) {
  expect(pageText.length, `${route} rendered too little visible text`).toBeGreaterThan(20);
  expect(hasArabic(pageText), `${route} did not render Arabic UI text`).toBeTruthy();
  for (const pattern of TECHNICAL_TEXT_PATTERNS) {
    expect(pageText, `${route} leaked technical text matching ${pattern}`).not.toMatch(pattern);
  }
}
...
test.describe("@ux-gate final Arabic user experience", () => {
  for (const route of criticalRoutes) {
    test(`critical route is Arabic/RTL and runtime-clean: ${route}`, async ({ page }) => {
      ...
      await expectArabicRtlOperationalPage(bodyText, route);
  ...
  test("mobile shell remains usable on Arabic critical entry @mobile", async ({ page }) => {
    ...
    if (response) {
      expect(response.status(), `mobile / returned HTTP ${response.status()}`).toBeLessThan(400);
    }

    await page.locator("body").waitFor({ state: "visible" });
    const bodyText = (await page.locator("body").innerText()).replace(/\s+/g, " ").trim();
    await expectArabicRtlOperationalPage(bodyText, "mobile /");
